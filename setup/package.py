...
class Docker():
    ...
    def __init__(self, args):
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER flectra', DOCKERUSER)
...
class DockerTgz(Docker):
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su flectra -s /bin/bash -c "python3 -m venv /var/lib/flectra/flectravenv"',
            'su flectra -s /bin/bash -c "/var/lib/flectra/flectravenv/bin/python3 -m pip install --upgrade pip"',
            'su flectra -s /bin/bash -c "/var/lib/flectra/flectravenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su flectra -s /bin/bash -c "/var/lib/flectra/flectravenv/bin/python3 -m pip install /data/src/flectra_{VERSION}.{TSTAMP}.tar.gz"',
            'su flectra -s /bin/bash -c "createdb mycompany"',
            'su flectra -s /bin/bash -c "/var/lib/flectra/flectravenv/bin/flectra -d mycompany -i base --stop-after-init"',
            'su flectra -s /bin/bash -c "/var/lib/flectra/flectravenv/bin/flectra -d mycompany --pidfile=/data/src/flectra.pid"',
        ]
...
class DockerDeb(Docker):
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/flectra_{VERSION}.{TSTAMP}_all.deb',
            'su flectra -s /bin/bash -c "flectra -d mycompany -i base --pidfile=/data/src/flectra.pid"',
        ]
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/flectra/rpmbuild'
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/flectra.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./flectra-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/flectra-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" /data/src/setup/rpm/flectra.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/flectra*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'flectra-rpm-build-{TSTAMP}')
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su flectra -c "createdb mycompany"',
...
class DockerWine(Docker):
    ...
    def build(self):
        ...
        container_python = '/var/lib/flectra/.wine/drive_c/flectrabuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        cmds = [
            ...
            rf'wine "c:\nsis-3.10\makensis.exe" {nsis_args} "c:\flectrabuild\server\setup\win32\setup.nsi"',
            rf'wine {container_python} -m pip list'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'flectra-win-build-%s' % TSTAMP)
