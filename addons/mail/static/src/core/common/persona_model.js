...
export class Persona extends Record {
    ...
    storeAsTrackedImStatus = Record.one("Store", {
        ...
        onAdd() {
            ...
            this._store.env.services.bus_service.addChannel(`flectra-presence-${model}_${this.id}`);
        },
        onDelete() {
            if (!this._store.env.services.bus_service.isActive) {
                return;
            ...
            const model = this.type === "partner" ? "res.partner" : "mail.guest";
            this._store.env.services.bus_service.deleteChannel(`flectra-presence-${model}_${this.id}`);
        },
        eager: true,
