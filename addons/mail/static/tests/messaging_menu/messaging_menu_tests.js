...
QUnit.test("rendering with FlectraBot has a request (default)", async (assert) => {
    ...
    await contains(".o-mail-NotificationItem", { text: "FlectraBot has a request" });
});

QUnit.test("rendering without FlectraBot has a request (denied)", async () => {
    patchBrowserNotification("denied");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
...
QUnit.test("rendering without FlectraBot has a request (accepted)", async () => {
...
QUnit.test("respond to notification prompt (denied)", async () => {
    ...
    await contains(".o_notification.border-warning", {
        text: "Flectra will not send notifications on this device.",
    });
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-NotificationItem", { count: 0 });
...
QUnit.test("respond to notification prompt (granted)", async () => {
    ...
    await contains(".o_notification.border-success", {
        text: "Flectra will send notifications on this device!",
    });
});

QUnit.test("no 'FlectraBot has a request' in mobile app", async () => {
    patchBrowserNotification("default");
    // simulate Android Flectra App
    patchWithCleanup(browser.navigator, {
        userAgent: "Chrome/0.0.0 Android (FlectraMobile; Linux; Android 13; Flectra TestSuite)",
    });
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
...
QUnit.test("rendering with PWA installation request", async (assert) => {
    ...
    assert.strictEqual(
        ...
        "FlectraBot has a suggestion"
    );
    assert.strictEqual(
        target.querySelector(".o-mail-NotificationItem-text").textContent,
        "Come here often? Install Flectra on your device!"
    );
