...
QUnit.test("addLink: utility function and special entities", function (assert) {
    const testInputs = {
        ...
        "https://flectrahq.com/%5B%5D": `<a target="_blank" rel="noreferrer noopener" href="https://flectrahq.com/%5B%5D">https://flectrahq.com/[]</a>`,
    };
...
QUnit.test("url", async () => {
    ...
    const messageBody = "https://flectrahq.com?test=~^|`{}[]#";
...
QUnit.test("url with comma at the end", async () => {
    ...
    const { openDiscuss } = await start();
    await openDiscuss(channelId);
    const messageBody = "Go to https://flectrahq.com, it's great!";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button:enabled", { text: "Send" });
    await contains(".o-mail-Message a", { text: "https://flectrahq.com" });
...
QUnit.test("url with number in subdomain", async () => {
    ...
    const { openDiscuss } = await start();
    await openDiscuss(channelId);
    const messageBody = "https://www.45017478-master-all.runbot134.flectrahq.com/web";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button:enabled", { text: "Send" });
    await contains(".o-mail-Message a", {
        text: "https://www.45017478-master-all.runbot134.flectrahq.com/web",
    });
});
