...
QUnit.test("subscribe to known partner presences", async () => {
    ...
    const expectedPresences = [
        `flectra-presence-res.partner_${pyEnv.flectrabotId}`,
        `flectra-presence-res.partner_${pyEnv.currentPartnerId}`,
    ];
    ...
    expectedPresences.push(`flectra-presence-res.partner_${bobPartnerId}`);
    await assertSteps([`subscribe - [${expectedPresences.join(",")}]`]);
});
