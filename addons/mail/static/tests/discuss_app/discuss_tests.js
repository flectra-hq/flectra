...
QUnit.test("Posting message should transform links.", async () => {
    ...
    const { openDiscuss } = await start();
    openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", "test https://www.flectrahq.com/");
    await click(".o-mail-Composer-send:enabled");
    await contains("a[href='https://www.flectrahq.com/']");
});
...
QUnit.test("receive new chat messages: out of flectra focus (tab title)", async (assert) => {
    let stepCount = 0;
    const pyEnv = await startServer();
    ...
    // simulate receiving a new message in chat 1 with flectra out-of-focused
    ...
    // simulate receiving a new message in chat 2 with flectra out-of-focused
    ...
    // simulate receiving another new message in chat 2 with flectra focused
    pyEnv["bus.bus"]._sendone(channel_2, "discuss.channel/new_message", {
        id: channelId_2,
        message: {
            id: 128,
    ...
    async () => {
        ...
        pyEnv["discuss.channel"].create([
            ...
            },
            {
                channel_member_ids: [
                    Command.create({ partner_id: pyEnv.currentPartnerId }),
                    Command.create({ partner_id: TEST_USER_IDS.flectrabotId }),
                ],
                channel_type: "chat",
            },
        ]);
        ...
        await click(".o-mail-DiscussSidebarChannel", { text: "FlectraBot" });
