...
class Partner(models.Model):
    ...
    def get_mention_suggestions(self, search, limit=8):
        ...
        # add FlectraBot even if its partner is archived
        if len(partners) + len(users) < limit and "flectrabot".startswith(search.lower()):
            flectrabot = self.env.ref("base.partner_root")
            if not any(elem['id'] == flectrabot.id for elem in partners):
                partners.append(flectrabot.read(fields)[0])
