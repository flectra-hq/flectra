...
from flectra.addons.payment_mercado_pago import const
...
class PaymentTransaction(models.Model):
    ...
    def _process_notification_data(self, notification_data):
        ...
        for flectra_code, mp_codes in const.PAYMENT_METHODS_MAPPING.items():
            if any(payment_method_type == mp_code for mp_code in mp_codes.split(',')):
                payment_method_type = flectra_code
                break
        payment_method = self.env['payment.method']._get_from_code(
            payment_method_type, mapping=const.PAYMENT_METHODS_MAPPING
        )
        # Fall back to "unknown" if the payment method is not found (and if "unknown" is found), as
        # the user might have picked a different payment method than on Flectra's payment form.
