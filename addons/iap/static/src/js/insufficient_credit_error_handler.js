...
import { Component, onWillStart } from "@flectra/owl";
...
class InsufficientCreditDialog extends Component {
    ...
    async onWillStart() {
        ...
        const { isEnterprise } = flectra.info;
        if (errorData.trial && isEnterprise) {
            this.buttonMessage = _t("Start a Trial at Flectra");
        } else {
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "flectra.addons.iap.tools.iap_tools.InsufficientCreditError") {
