...
class MicrosoftSync(models.AbstractModel):
    ...
    def _update_microsoft_recurrence(self, recurrence_event, events):
        ...
        for recurrent_event in events_to_update:
            if recurrent_event.type == 'occurrence':
                ...
                value = self.env['calendar.event']._microsoft_to_flectra_values(recurrent_event, (), default_values)
                self.env['calendar.event'].browse(recurrent_event.flectra_id(self.env)).with_context(no_mail_to_attendees=True, mail_create_nolog=True).write(dict(value, need_sync_m=False))
    ...
    def _sync_microsoft2flectra(self, microsoft_events: MicrosoftEvent, default_reminders=()):
        ...
        new_flectra = self.with_context(dont_notify=True).create(flectra_values)

        synced_recurrent_records = self.with_context(dont_notify=True)._sync_recurrence_microsoft2flectra(new_recurrent)
    ...
    def _notify_attendees(self):
        ...
        This is called when creating new calendar events in _sync_microsoft2flectra.
