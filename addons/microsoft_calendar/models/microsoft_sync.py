...
class MicrosoftSync(models.AbstractModel):
    ...
    def _sync_recurrence_microsoft2flectra(self, microsoft_events, new_events=None):
        ...
        for recurrent_master in recurrent_masters:
            ...
            new_recurrence_flectra = self.env['calendar.recurrence'].with_context(dont_notify=True).create(new_calendar_recurrence)
    ...
    def _update_microsoft_recurrence(self, recurrence, events):
        ...
        for e in events_to_update:
            ...
            if event_values:
                ...
                flectra_event.with_context(dont_notify=True).write(dict(event_values, need_sync_m=False))
    ...
    def _sync_microsoft2flectra(self, microsoft_events: MicrosoftEvent):
        ...
        # Get sync lower bound days range for checking if old events must be updated in Flectra.
        ...
        for mevent in (existing - cancelled).filter(lambda e: e.lastModifiedDateTime):
            ...
            if flectra_event:
                ...
                if lower_bound_day_range:
                    update_time_diff = ms_event_updated_time - flectra_event_updated_time
                    old_event_update_condition = flectra_event._check_old_event_update_required(int(lower_bound_day_range), update_time_diff)

                if ms_event_updated_time >= flectra_event_updated_time and old_event_update_condition:
                    vals = dict(flectra_event._microsoft_to_flectra_values(mevent), need_sync_m=False)
                    flectra_event.with_context(dont_notify=True)._write_from_microsoft(mevent, vals)
    ...
    def _check_old_event_update_required(self, lower_bound_day_range, update_time_diff):
        """
        Checks if an old event in Flectra should be updated locally. This verification is necessary because
        sometimes events in Flectra have the same state in Microsoft and even so they trigger updates locally
    ...
    def _microsoft_attendee_answer(self, answer, params, timeout=TIMEOUT):
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                # former differs for each attendee. This info is required for sending the event answer and Flectra currently
