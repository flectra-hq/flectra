...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A Flectra event is created when Outlook sync is not enabled.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_flectra(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Flectra attendee calendar.
    ...
    def test_create_simple_event_from_outlook_attendee_calendar_where_email_addresses_are_capitalized(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Flectra attendee calendar.
        The email addresses of the attendee and the organizer are in different case than in Flectra.
        """
    ...
    def test_create_recurrent_event_without_sync(self, mock_insert):
        """
        A Flectra recurrent event is created when Outlook sync is not enabled.
        """
        if not self.sync_flectra_recurrences_with_outlook_feature():
            return
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in Flectra due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in Flectra Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its Flectra Calendar synced.
        ...
        # In Flectra, this event will be created (behind the screens) by a synced Flectra user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with Flectra through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in Flectra (no ACL error was triggered blocking creation).
    ...
    def test_create_simple_event_from_outlook_without_organizer(self, mock_get_events):
        """
        Allow creation of an event without organizer in Outlook and sync it in Flectra.
        """
    ...
    def test_skip_sync_for_non_synchronized_users_new_events(self, mock_insert):
        ...
        Flectra, as well cause problems in the future the synchronization of that event for the original owner.
        """
        ...
        self.assertTrue(event, "The event for the not synchronized owner must be created in Flectra.")

        # Synchronize the calendar of user A, then make sure insert was not called.
        event.with_user(self.organizer_user).sudo()._sync_flectra2microsoft()
        mock_insert.assert_not_called()
    ...
    def test_create_duplicate_event_microsoft_calendar(self, mock_insert, mock_get_events):
        """
        Test syncing an event from Flectra to Microsoft Calendar.
        """
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        ...
        self.response_from_outlook_organizer = {
            **self.simple_event_from_outlook_organizer,
            '_flectra_id': record.id,
        }
        self.response_from_outlook_organizer_1 = {
            **self.simple_event_from_outlook_organizer,
            '_flectra_id': record2.id,
        }
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    @patch.object(MicrosoftCalendarService, 'insert')
    def test_new_db_skip_flectra2microsoft_sync_previously_created_events(self, mock_insert, mock_get_events):
        """
        ...
        # Ten seconds later the ICP parameter saving, make the synchronization between Flectra
        ...
        with self.mock_datetime_and_now('2024-01-02 10:00:10'):
            # Mock the return of 0 events from Outlook to Flectra, then activate the user's sync.
            mock_get_events.return_value = ([], None)
            self.organizer_user.microsoft_synchronization_stopped = False
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    @patch.object(MicrosoftCalendarService, 'insert')
    def test_old_db_flectra2microsoft_sync_previously_created_events(self, mock_insert, mock_get_events):
        """
