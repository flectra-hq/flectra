...
xml_id_url_prefix = "flectra://ir_menu_xml_id/"

flectra_view_link_prefix = "flectra://view/"


def flectra_charts(data):
    """return all flectra chart definition in the spreadsheet"""
    figures = []
    for sheet in data["sheets"]:
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet["figures"]
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("flectra_")
        ]
    return figures
...
def flectra_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like flectra://view/{... view data...}
    """
    return [
        json.loads(url[len(flectra_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(flectra_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = flectra_charts(data)
    flectra_version = data.get("flectraVersion", 1)
    if flectra_version < 5:
...
def fields_in_spreadsheet(data):
    ...
    all_fields = chain(
        ...
        extract_fields(chart_fields, flectra_charts(data)).items(),
        extract_fields(flectra_view_fields, flectra_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartFlectraMenusReferences", {}).values()) | {
