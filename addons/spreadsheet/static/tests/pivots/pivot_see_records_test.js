...
QUnit.test("Cannot open see records on the main FLECTRA.PIVOT.TABLE cell", async function (assert) {
    const { env, model } = await createSpreadsheetWithPivot();
...
QUnit.test(
    "Cannot open see records on the empty FLECTRA.PIVOT.TABLE cell below the main cell",
...
QUnit.test(
    ...
    async function (assert) {
        ...
        model.dispatch("CREATE_SHEET", { sheetId: "42" });
        // FLECTRA.PIVOT.TABLE(1) would have 2 columns and 7 rows
        // TRANSPOSE(FLECTRA.PIVOT.TABLE(1)) has 7 columns and 2 rows
        setCellContent(model, "A1", "=TRANSPOSE(FLECTRA.PIVOT.TABLE(1))", "42");
...
QUnit.test("Can see records on FLECTRA.PIVOT.TABLE cells", async function (assert) {
    const actions = [];
    const fakeActionService = {
        start: (env) => ({
            doAction: (actionRequest, options = {}) => {
    ...
    async function checkCells(cells) {
        ...
        // sheet where the pivot is made of a single FLECTRA.PIVOT.TABLE formula.
        for (const [xc, formula] of Object.entries(cells)) {
            ...
            // action on the second sheet, on FLECTRA.PIVOT.TABLE
    ...
    const header_cells = {
        ...
        B1: '=FLECTRA.PIVOT.HEADER(1,"foo",1)',
        // B2 is a measure header
        B2: '=FLECTRA.PIVOT.HEADER(1,"foo",1,"measure","probability")',
        // A3 is a row header
        A3: '=FLECTRA.PIVOT.HEADER(1,"bar","false")',
        // A5 is a total header
        A5: "=FLECTRA.PIVOT.HEADER(1)",
    };
    const data_cells = {
        // B3 is an empty value
        B3: '=FLECTRA.PIVOT(1,"probability","bar","false","foo",1)',
        // B4 is an non-empty value
        B4: '=FLECTRA.PIVOT(1,"probability","bar","true","foo",1)',
        // B5 is a column group total value
        B5: '=FLECTRA.PIVOT(1,"probability","foo",1)',
        // F3 is a row group total value
        F3: '=FLECTRA.PIVOT(1,"probability","bar","false")',
        // F5 is the total
        F5: '=FLECTRA.PIVOT(1,"probability")',
    };
    ...
    setCellContent(model, "A3", `=FLECTRA.PIVOT.TABLE("1",,,FALSE)`, "42");
...
QUnit.test(
    ...
    async function (assert) {
        ...
        setCellContent(model, "A1", '=IFERROR(FLECTRA.PIVOT("1","probability"), 42)');
...
QUnit.test("See records is not visible if the formula has an weird IF", async function (assert) {
    ...
    setCellContent(
        model,
        "A1",
        '=if(false, FLECTRA.PIVOT("1","probability","user_id",2,"partner_id", "#Error"), "test")'
    );
    deferred = makeDeferred();
    model.dispatch("REFRESH_ALL_DATA_SOURCES");
    const action = cellMenuRegistry.getAll().find((item) => item.id === "pivot_see_records");
