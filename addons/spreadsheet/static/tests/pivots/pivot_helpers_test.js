...
import { constants, tokenize } from "@flectra/o-spreadsheet";
...
QUnit.module("spreadsheet > pivot_helpers", {}, () => {
    QUnit.test("Basic formula extractor", async function (assert) {
        const formula = `=FLECTRA.PIVOT("1", "test") + FLECTRA.LIST("2", "hello", "bla")`;
        const tokens = tokenize(formula);
        let functionName;
        let args;
        ({ functionName, args } = getFirstPivotFunction(tokens));
        assert.strictEqual(functionName, "FLECTRA.PIVOT");
        ...
        assert.strictEqual(functionName, "FLECTRA.LIST");
    ...
    QUnit.test("Extraction with two PIVOT formulas", async function (assert) {
        const formula = `=FLECTRA.PIVOT("1", "test") + FLECTRA.PIVOT("2", "hello", "bla")`;
        const tokens = tokenize(formula);
    ...
    QUnit.test("Number of formulas", async function (assert) {
        const formula = `=FLECTRA.PIVOT("1", "test") + FLECTRA.PIVOT("2", "hello", "bla") + FLECTRA.LIST("1", "bla")`;
