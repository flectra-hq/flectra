...
QUnit.module("spreadsheet > pivot plugin", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithPivot();
            setCellContent(model, "C3", `=FLECTRA.PIVOT(G10,"probability","bar","false","foo","2")+2`);
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "A2", '=FLECTRA.PIVOT(A1,"probability")');
    ...
    QUnit.test("Can remove a pivot with undo after editing a cell", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.ok(getCellContent(model, "B1").startsWith("=FLECTRA.PIVOT.HEADER"));
    ...
    QUnit.test("rename pivot with empty name is refused", async (assert) => {
        const { model } = await createSpreadsheetWithPivot();
        const result = model.dispatch("RENAME_FLECTRA_PIVOT", {
            pivotId: "1",
            name: "",
        });
    ...
    QUnit.test("Undo/Redo for RENAME_FLECTRA_PIVOT", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.equal(model.getters.getPivotName("1"), "Partner Pivot");
        model.dispatch("RENAME_FLECTRA_PIVOT", { pivotId: "1", name: "test" });
    ...
    QUnit.test("Format header displays an error for non-existing field", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "G10", `=FLECTRA.PIVOT.HEADER("1", "measure", "non-existing")`);
        setCellContent(model, "G11", `=FLECTRA.PIVOT.HEADER("1", "non-existing", "bla")`);
        await nextTick();
            ...
            sheets: [
                {
                    ...
                    cells: {
                        A1: { content: `=FLECTRA.PIVOT(1, "probability")` },
                        A2: { content: `=FLECTRA.PIVOT(2, "probability")` },
                    },
                },
            ],
            pivots: {
    ...
    QUnit.test("evaluates only once when two pivots are loading", async function (assert) {
        ...
        setCellContent(model, "A1", '=FLECTRA.PIVOT("1", "probability")');
        setCellContent(model, "A2", '=FLECTRA.PIVOT("2", "probability")');
        assert.equal(getCellValue(model, "A1"), "Loading...");
            ...
            sheets: [
                {
                    id: "sheet1",
                    cells: {
                        A1: { content: `=FLECTRA.PIVOT.HEADER(1, "measure", "probability")` },
                        A2: { content: `=FLECTRA.PIVOT.HEADER(1, "product_id", 37)` },
                        A3: { content: `=FLECTRA.PIVOT(1, "probability")` },
                    },
                },
            ],
            pivots: {
    ...
    QUnit.test("pivot grouped by char field which represents numbers", async function (assert) {
        ...
        assert.strictEqual(getCell(model, "A3").content, '=FLECTRA.PIVOT.HEADER(1,"name","000111")');
        assert.strictEqual(getCell(model, "A4").content, '=FLECTRA.PIVOT.HEADER(1,"name",111)');
        ...
        assert.strictEqual(
            ...
            '=FLECTRA.PIVOT(1,"probability","name","000111")'
        );
        assert.strictEqual(getCell(model, "B4").content, '=FLECTRA.PIVOT(1,"probability","name",111)');
    ...
    QUnit.test("relational PIVOT.HEADER with missing id", async function (assert) {
        ...
        model.dispatch("UPDATE_CELL", {
            col: 4,
            row: 9,
            content: `=FLECTRA.PIVOT.HEADER("1", "product_id", "1111111")`,
            sheetId,
        });
        await waitForDataSourcesLoaded(model);
        assert.equal(
    ...
    QUnit.test("relational PIVOT.HEADER with undefined id", async function (assert) {
        ...
        setCellContent(model, "F10", `=FLECTRA.PIVOT.HEADER("1", "product_id", A25)`);
    ...
    QUnit.test("aggregate to 0", async function (assert) {
        ...
        setCellContent(model, "A1", '=FLECTRA.PIVOT(1, "probability", "name", "A")');
        setCellContent(model, "A2", '=FLECTRA.PIVOT(1, "probability", "name", "B")');
        setCellContent(model, "A3", '=FLECTRA.PIVOT(1, "probability")');
    ...
    QUnit.test("can import (export) contextual domain", async (assert) => {
        ...
        setCellContent(model, "A1", '=FLECTRA.PIVOT(1, "probability")'); // load the data (and check the rpc domain)
        await nextTick();
        assert.strictEqual(
    ...
    QUnit.test("Can group by many2many field ", async (assert) => {
        ...
        assert.equal(getCellFormula(model, "A3"), '=FLECTRA.PIVOT.HEADER(1,"tag_ids","false")');
        assert.equal(getCellFormula(model, "A4"), '=FLECTRA.PIVOT.HEADER(1,"tag_ids",42)');
        assert.equal(getCellFormula(model, "A5"), '=FLECTRA.PIVOT.HEADER(1,"tag_ids",67)');

        assert.equal(
            getCellFormula(model, "B3"),
            '=FLECTRA.PIVOT(1,"probability","tag_ids","false","foo",1)'
        );
        assert.equal(
            getCellFormula(model, "B4"),
            '=FLECTRA.PIVOT(1,"probability","tag_ids",42,"foo",1)'
        );
        assert.equal(
            getCellFormula(model, "B5"),
            '=FLECTRA.PIVOT(1,"probability","tag_ids",67,"foo",1)'
        );

        assert.equal(
            getCellFormula(model, "C3"),
            '=FLECTRA.PIVOT(1,"probability","tag_ids","false","foo",2)'
        );
        assert.equal(
            getCellFormula(model, "C4"),
            '=FLECTRA.PIVOT(1,"probability","tag_ids",42,"foo",2)'
        );
        assert.equal(
            getCellFormula(model, "C5"),
            '=FLECTRA.PIVOT(1,"probability","tag_ids",67,"foo",2)'
        );
    ...
    QUnit.test("PIVOT.HEADER grouped by date field without value", async function (assert) {
        for (const interval of ["day", "week", "month", "quarter", "year"]) {
            ...
            setCellContent(model, "A1", `=FLECTRA.PIVOT.HEADER(1, "date:${interval}", "false")`);
