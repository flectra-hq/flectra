...
QUnit.module("spreadsheet > positional pivot formula", {}, () => {
    QUnit.test("Can have positional args in pivot formula", async function (assert) {
        ...
        // Columns
        setCellContent(model, "H1", `=FLECTRA.PIVOT(1,"probability","#foo", 1)`);
        setCellContent(model, "H2", `=FLECTRA.PIVOT(1,"probability","#foo", 2)`);
        setCellContent(model, "H3", `=FLECTRA.PIVOT(1,"probability","#foo", 3)`);
        setCellContent(model, "H4", `=FLECTRA.PIVOT(1,"probability","#foo", 4)`);
        setCellContent(model, "H5", `=FLECTRA.PIVOT(1,"probability","#foo", 5)`);
        ...
        // Rows
        setCellContent(model, "I1", `=FLECTRA.PIVOT(1,"probability","#bar", 1)`);
        setCellContent(model, "I2", `=FLECTRA.PIVOT(1,"probability","#bar", 2)`);
        setCellContent(model, "I3", `=FLECTRA.PIVOT(1,"probability","#bar", 3)`);
    ...
    QUnit.test("Can have positional args in pivot headers formula", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        // Columns
        setCellContent(model, "H1", `=FLECTRA.PIVOT.HEADER(1,"#foo",1)`);
        setCellContent(model, "H2", `=FLECTRA.PIVOT.HEADER(1,"#foo",2)`);
        setCellContent(model, "H3", `=FLECTRA.PIVOT.HEADER(1,"#foo",3)`);
        setCellContent(model, "H4", `=FLECTRA.PIVOT.HEADER(1,"#foo",4)`);
        setCellContent(model, "H5", `=FLECTRA.PIVOT.HEADER(1,"#foo",5)`);
        setCellContent(model, "H6", `=FLECTRA.PIVOT.HEADER(1,"#foo",5, "measure", "probability")`);
        ...
        // Rows
        setCellContent(model, "I1", `=FLECTRA.PIVOT.HEADER(1,"#bar",1)`);
        setCellContent(model, "I2", `=FLECTRA.PIVOT.HEADER(1,"#bar",2)`);
        setCellContent(model, "I3", `=FLECTRA.PIVOT.HEADER(1,"#bar",3)`);
        setCellContent(model, "I4", `=FLECTRA.PIVOT.HEADER(1,"#bar",3, "measure", "probability")`);
    ...
    QUnit.test("pivot positional with two levels of group bys in rows", async (assert) => {
        ...
        // Rows Headers
        setCellContent(model, "H1", `=FLECTRA.PIVOT.HEADER(1,"bar","false","#product_id",1)`);
        setCellContent(model, "H2", `=FLECTRA.PIVOT.HEADER(1,"bar","true","#product_id",1)`);
        setCellContent(model, "H3", `=FLECTRA.PIVOT.HEADER(1,"#bar",1,"#product_id",1)`);
        setCellContent(model, "H4", `=FLECTRA.PIVOT.HEADER(1,"#bar",2,"#product_id",1)`);
        setCellContent(model, "H5", `=FLECTRA.PIVOT.HEADER(1,"#bar",3,"#product_id",1)`);
        ...
        setCellContent(
            model,
            "H1",
            `=FLECTRA.PIVOT(1,"probability","#bar",1,"#product_id",1,"#foo",2)`
        );
        setCellContent(
            model,
            "H2",
            `=FLECTRA.PIVOT(1,"probability","#bar",1,"#product_id",2,"#foo",2)`
        );
    ...
    QUnit.test("Positional argument without a number should crash", async (assert) => {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "A10", `=FLECTRA.PIVOT.HEADER(1,"#bar","this is not a number")`);
        assert.strictEqual(getCellValue(model, "A10"), "#ERROR");
        assert.strictEqual(
            getEvaluatedCell(model, "A10").error.message,
            "The function FLECTRA.PIVOT.HEADER expects a number value, but 'this is not a number' is a string, and cannot be coerced to a number."
        );
    });
    ...
    QUnit.test("sort first pivot column (ascending)", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.PIVOT.HEADER(1,"#bar",1)`);
        setCellContent(model, "A2", `=FLECTRA.PIVOT.HEADER(1,"#bar",2)`);
        setCellContent(model, "B1", `=FLECTRA.PIVOT(1,"probability","#bar",1,"#foo",1)`);
        setCellContent(model, "B2", `=FLECTRA.PIVOT(1,"probability","#bar",2,"#foo",1)`);
        setCellContent(model, "C1", `=FLECTRA.PIVOT(1,"probability","#bar",1,"#foo",2)`);
        setCellContent(model, "C2", `=FLECTRA.PIVOT(1,"probability","#bar",2,"#foo",2)`);
        setCellContent(model, "D1", `=FLECTRA.PIVOT(1,"probability","#bar",1)`);
        setCellContent(model, "D2", `=FLECTRA.PIVOT(1,"probability","#bar",2)`);
    ...
    QUnit.test("sort second pivot measure (ascending)", async (assert) => {
        ...
        const model = await createModelWithDataSource({ spreadsheetData });
        setCellContent(model, "A10", `=FLECTRA.PIVOT.HEADER(1,"#product_id",1)`);
        setCellContent(model, "A11", `=FLECTRA.PIVOT.HEADER(1,"#product_id",2)`);
        setCellContent(model, "B10", `=FLECTRA.PIVOT(1,"probability","#product_id",1)`);
        setCellContent(model, "B11", `=FLECTRA.PIVOT(1,"probability","#product_id",2)`);
        setCellContent(model, "C10", `=FLECTRA.PIVOT(1,"foo","#product_id",1)`);
        setCellContent(model, "C11", `=FLECTRA.PIVOT(1,"foo","#product_id",2)`);
        await waitForDataSourcesLoaded(model);
    ...
    QUnit.test("Formatting a pivot positional preserves the interval", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.PIVOT.HEADER(1,"#date:day",1)`);
