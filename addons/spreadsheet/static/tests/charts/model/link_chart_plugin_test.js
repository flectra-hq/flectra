    ...
    () => {
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.equal(
                    exportedData.chartFlectraMenusReferences[chartId],
                    1,
                    "Link to flectra menu is exported"
                );
                const importedModel = new Model(exportedData, { custom: { env } });
                const chartMenu = importedModel.getters.getChartFlectraMenu(chartId);
                assert.equal(chartMenu.id, 1, "Link to flectra menu is imported");
            }
        );

        QUnit.test("Can undo-redo a LINK_FLECTRA_MENU_TO_CHART", async function (assert) {
            const env = await makeTestEnv({ serverData: this.serverData });
            const model = new Model({}, { custom: { env } });
            createBasicChart(model, chartId);
            model.dispatch("LINK_FLECTRA_MENU_TO_CHART", {
            ...
            assert.equal(model.getters.getChartFlectraMenu(chartId).id, 1);
