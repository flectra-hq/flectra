...
import { FlectraBarChart } from "@spreadsheet/chart/flectra_chart/flectra_bar_chart";
import { FlectraChart } from "@spreadsheet/chart/flectra_chart/flectra_chart";
import { FlectraLineChart } from "@spreadsheet/chart/flectra_chart/flectra_line_chart";
...
QUnit.module("spreadsheet > flectra chart plugin", {}, () => {
    QUnit.test("Can add an Flectra Bar chart", async (assert) => {
        ...
        assert.ok(chart instanceof FlectraBarChart);
    ...
    QUnit.test("Can add an Flectra Line chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_line" });
        const sheetId = model.getters.getActiveSheetId();
        assert.strictEqual(model.getters.getChartIds(sheetId).length, 1);
        const chartId = model.getters.getChartIds(sheetId)[0];
        const chart = model.getters.getChart(chartId);
        assert.ok(chart instanceof FlectraLineChart);
        assert.strictEqual(chart.getDefinitionForExcel(), undefined);
        assert.strictEqual(model.getters.getChartRuntime(chartId).chartJsConfig.type, "line");
    });

    QUnit.test("Can add an Flectra Pie chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_pie" });
        const sheetId = model.getters.getActiveSheetId();
        assert.strictEqual(model.getters.getChartIds(sheetId).length, 1);
        const chartId = model.getters.getChartIds(sheetId)[0];
        const chart = model.getters.getChart(chartId);
        assert.ok(chart instanceof FlectraChart);
        assert.strictEqual(chart.getDefinitionForExcel(), undefined);
    ...
    QUnit.test("Flectra bar chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "flectra_bar",
    ...
    QUnit.test("Flectra pie chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "flectra_pie",
            mockRPC: async function (route, args) {
                if (args.method === "web_read_group") {
    ...
    QUnit.test("Flectra line chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "flectra_line",
            mockRPC: async function (route, args) {
                if (args.method === "web_read_group") {
    ...
    QUnit.test("Can import/export an Flectra chart", async (assert) => {
        const model = await createModelWithDataSource();
        insertChartInSpreadsheet(model, "flectra_line");
        const data = model.exportData();
        const figures = data.sheets[0].figures;
        assert.strictEqual(figures.length, 1);
        const figure = figures[0];
        assert.strictEqual(figure.tag, "chart");
        assert.strictEqual(figure.data.type, "flectra_line");
    ...
    QUnit.test("Can undo/redo an Flectra chart creation", async (assert) => {
        const model = await createModelWithDataSource();
        insertChartInSpreadsheet(model, "flectra_line");
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    QUnit.test("charts with no legend", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_pie" });
        insertChartInSpreadsheet(model, "flectra_bar");
        insertChartInSpreadsheet(model, "flectra_line");
        const sheetId = model.getters.getActiveSheetId();
    ...
    QUnit.test("Can copy/paste Flectra chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_pie" });
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
        ...
        assert.ok(model.getters.getChart(chartIds[1]) instanceof FlectraChart);
        assert.strictEqual(
    ...
    QUnit.test("Can cut/paste Flectra chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_pie" });
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
        ...
        assert.ok(model.getters.getChart(chartIds[0]) instanceof FlectraChart);
        assert.strictEqual(
    ...
    QUnit.test("Duplicating a sheet correctly duplicates Flectra chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_bar" });
        const sheetId = model.getters.getActiveSheetId();
    ...
    QUnit.test(
        "Load flectra chart spreadsheet with models that cannot be accessed",
    ...
    QUnit.test("Can insert flectra chart from a different model", async (assert) => {
        const model = await createModelWithDataSource();
    ...
    QUnit.test("Remove flectra chart when sheet is deleted", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_line" });
        const sheetId = model.getters.getActiveSheetId();
        ...
        assert.strictEqual(model.getters.getFlectraChartIds().length, 1);
        model.dispatch("DELETE_SHEET", { sheetId });
        assert.strictEqual(model.getters.getFlectraChartIds().length, 0);
    });

    QUnit.test("Flectra chart legend color changes with background color update", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_bar" });
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
        const definition = model.getters.getChartDefinition(chartId);
    ...
    QUnit.test(
        "Displays correct thousand separator for positive value in Flectra Bar chart Y-axis",
        async (assert) => {
            const { model } = await createSpreadsheetWithChart({ type: "flectra_bar" });
            const sheetId = model.getters.getActiveSheetId();
            const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    QUnit.test(
        "Thousand separator in Flectra Bar chart Y-axis is locale-dependent",
        async (assert) => {
            const { model } = await createSpreadsheetWithChart({ type: "flectra_bar" });
