...
import { Model, DispatchResult, helpers, tokenize } from "@flectra/o-spreadsheet";
import {
...
QUnit.module("spreadsheet > Global filters model", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const chartId = model.getters.getFlectraChartIds()[0];
            fieldMatching = model.getters.getFlectraChartFieldMatching(chartId, filterId);
            assert.equal(fieldMatching.chain, "created_on");
            assert.equal(fieldMatching.type, "date");
            assert.equal(fieldMatching.offset, undefined);
        }
    ...
    QUnit.test("FLECTRA.FILTER.VALUE text filter", async function (assert) {
        assert.expect(3);

        const model = await createModelWithDataSource();
        setCellContent(model, "A10", `=FLECTRA.FILTER.VALUE("Text Filter")`);
        await nextTick();
    ...
    QUnit.test("FLECTRA.FILTER.VALUE date filter", async function (assert) {
        assert.expect(4);

        const model = await createModelWithDataSource();
        setCellContent(model, "A10", `=FLECTRA.FILTER.VALUE("Date Filter")`);
        await nextTick();
        await addGlobalFilter(model, {
            id: "42",
            type: "date",
    ...
    QUnit.test("FLECTRA.FILTER.VALUE date from/to without values", async function (assert) {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=FLECTRA.FILTER.VALUE("Date Filter")`);
        await addGlobalFilter(model, {
            id: "42",
            type: "date",
            label: "Date Filter",
    ...
    QUnit.test("FLECTRA.FILTER.VALUE date from/to with only from defined", async function (assert) {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=FLECTRA.FILTER.VALUE("Date Filter")`);
        await addGlobalFilter(model, {
            id: "42",
    ...
    QUnit.test("FLECTRA.FILTER.VALUE date from/to with only to defined", async function (assert) {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=FLECTRA.FILTER.VALUE("Date Filter")`);
        await addGlobalFilter(model, {
            id: "42",
    ...
    QUnit.test("FLECTRA.FILTER.VALUE date from/to with from and to defined", async function (assert) {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=FLECTRA.FILTER.VALUE("Date Filter")`);
        await addGlobalFilter(model, {
            id: "42",
    ...
    QUnit.test("FLECTRA.FILTER.VALUE relation filter", async function (assert) {
        const model = await createModelWithDataSource({
        ...
        setCellContent(model, "A10", `=FLECTRA.FILTER.VALUE("Relation Filter")`);
        await nextTick();
        await addGlobalFilter(model, {
            id: "42",
            type: "relation",
    ...
    QUnit.test(
        "FLECTRA.FILTER.VALUE with escaped quotes in the filter label",
        async function (assert) {
            const model = await createModelWithDataSource();
            await addGlobalFilter(model, {
                id: "42",
            ...
            setCellContent(model, "A1", '=FLECTRA.FILTER.VALUE("my \\"special\\" filter")');
            assert.equal(getCellValue(model, "A1"), "Jean-Jacques");
        }
    );

    QUnit.test(
        "FLECTRA.FILTER.VALUE formulas are updated when filter label is changed",
        async function (assert) {
            assert.expect(1);
            ...
            setCellContent(
                model,
                "A10",
                `=FLECTRA.FILTER.VALUE("Cuillère") & FLECTRA.FILTER.VALUE( "Cuillère" )`
            );
            const [filter] = model.getters.getGlobalFilters();
            const newFilter = {
                ...filter,
            ...
            assert.equal(
                ...
                `=FLECTRA.FILTER.VALUE("Interprete") & FLECTRA.FILTER.VALUE("Interprete")`
            );
        }
    );
                ...
                sheets: [
                    {
                        id: "sheet1",
                        cells: {
                            A1: { content: `=FLECTRA.PIVOT("1", "probability")` },
                        },
                    },
                ],
                pivots: {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const dateFilters2 = getFiltersMatchingPivot(
                model,
                '=FLECTRA.PIVOT.HEADER(1,"date:year","2016")'
            );
    ...
    QUnit.test("getFiltersMatchingPivot works with multiple inputs", async function (assert) {
        ...
        const matchingYearForQuarterGranularity = getFiltersMatchingPivot(
            model,
            '=FLECTRA.PIVOT.HEADER(1,"date:month","Q2/2024")'
        );
        ...
        const emptyString = getFiltersMatchingPivot(model, '=FLECTRA.PIVOT.HEADER(1,"date:month","")');
        assert.deepEqual(emptyString, [{ filterId: "43", value: undefined }]);

        const booleanValue = getFiltersMatchingPivot(
            model,
            '=FLECTRA.PIVOT.HEADER(1,"date:month",true)'
        );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const filters = getFiltersMatchingPivot(
                model,
                '=FLECTRA.PIVOT.HEADER(1,"date:month","08/2016","product_id","41")'
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=FLECTRA.PIVOT(1, "__count", "#product_id", 1)');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=FLECTRA.PIVOT(1, "probability", "#product_id", 1)');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=FLECTRA.PIVOT(1, "probability")');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test("field matching is removed when list is deleted", async function (assert) {
        ...
        model.dispatch("REMOVE_FLECTRA_LIST", { listId });
        assert.throws(
    ...
    QUnit.test("field matching is removed when an Flectra chart is deleted", async function (assert) {
        const { model } = await createSpreadsheetWithChart({ type: "flectra_pie" });
        const sheetId = model.getters.getActiveSheetId();
        ...
        assert.deepEqual(model.getters.getFlectraChartFieldMatching(chartId, filter.id), matching);
        model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
        assert.throws(
            () => model.getters.getFlectraChartFieldMatching(chartId, filter.id),
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const [pivotId] = model.getters.getPivotIds();
            model.dispatch("UPDATE_FLECTRA_PIVOT_DOMAIN", {
                pivotId,
                domain: [["foo", "in", [55]]],
            });
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const [listId] = model.getters.getListIds();
            model.dispatch("UPDATE_FLECTRA_LIST_DOMAIN", {
                listId,
                domain: [["foo", "in", [55]]],
            });
