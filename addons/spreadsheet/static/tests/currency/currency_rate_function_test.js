...
QUnit.module("spreadsheet > FLECTRA.CURRENCY.RATE function");
...
QUnit.test("Basic exchange formula", async (assert) => {
    ...
    setCellContent(model, "A1", `=FLECTRA.CURRENCY.RATE("EUR","USD")`);
...
QUnit.test("rate formula at a given date(time)", async (assert) => {
    ...
    setCellContent(model, "A1", `=FLECTRA.CURRENCY.RATE("EUR","USD", "12-31-2020")`);
    setCellContent(model, "A2", `=FLECTRA.CURRENCY.RATE("EUR","USD", "11-30-2020 00:00:00")`);
    await waitForDataSourcesLoaded(model);
    assert.verifySteps(["rate fetched"]);
});
...
QUnit.test("invalid date", async (assert) => {
    ...
    setCellContent(model, "A1", `=FLECTRA.CURRENCY.RATE("EUR","USD", "hello")`);
    await waitForDataSourcesLoaded(model);
    assert.strictEqual(getCellValue(model, "A1"), "#ERROR");
    assert.strictEqual(
        getEvaluatedCell(model, "A1").error.message,
        "The function FLECTRA.CURRENCY.RATE expects a number value, but 'hello' is a string, and cannot be coerced to a number."
    );
});
...
QUnit.test("Currency rate throw with unknown currency", async (assert) => {
    ...
    setCellContent(model, "A1", `=FLECTRA.CURRENCY.RATE("INVALID","USD")`);
    await waitForDataSourcesLoaded(model);
...
QUnit.test("Currency rates are only loaded once", async (assert) => {
    ...
    setCellContent(model, "A2", `=FLECTRA.CURRENCY.RATE("EUR","USD")`);
    await waitForDataSourcesLoaded(model);
    assert.verifySteps([]);
});
...
QUnit.test("Currency rates are loaded once by clock", async (assert) => {
    ...
    setCellContent(model, "A1", `=FLECTRA.CURRENCY.RATE("EUR","USD")`);
    setCellContent(model, "A2", `=FLECTRA.CURRENCY.RATE("EUR","SEK")`);
    await waitForDataSourcesLoaded(model);
