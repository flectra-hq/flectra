...
import { migrate, FLECTRA_VERSION } from "@spreadsheet/o_spreadsheet/migration";
import { Model } from "@flectra/o-spreadsheet";

QUnit.module("spreadsheet > migrations");

QUnit.test("Flectra formulas are migrated", (assert) => {
    const data = {
        sheets: [
            {
                cells: {
    ...
    assert.strictEqual(migratedData.sheets[0].cells.A1.content, `=FLECTRA.PIVOT("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A2.content, `=FLECTRA.PIVOT.HEADER("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=FLECTRA.FILTER.VALUE("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=FLECTRA.LIST("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A5.content, `=FLECTRA.LIST.HEADER("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A6.content, `=FLECTRA.PIVOT.POSITION("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A7.content, `=FLECTRA.PIVOT("1")`);
});
...
QUnit.test("Pivot 'day' arguments are migrated", (assert) => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=FLECTRA.PIVOT("1","21/07/2022")` },
                    A2: { content: `=FLECTRA.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=flectra.pivot("1","21/07/2021")` },
                    A4: { content: `=FLECTRA.PIVOT("1","test")` },
                    A5: { content: `=flectra.pivot("1","21/07/2021")+"21/07/2021"` },
    ...
    const migratedData = migrate(data);
    assert.strictEqual(migratedData.sheets[0].cells.A1.content, `=FLECTRA.PIVOT("1","07/21/2022")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A2.content,
        `=FLECTRA.PIVOT.HEADER("1","12/11/2022")`
    );
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=flectra.pivot("1","07/21/2021")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=FLECTRA.PIVOT("1","test")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A5.content,
        `=flectra.pivot("1","07/21/2021")+"21/07/2021"`
    );
...
QUnit.test("Flectra version is exported", (assert) => {
    const model = new Model();
    assert.strictEqual(model.exportData().flectraVersion, FLECTRA_VERSION);
});
