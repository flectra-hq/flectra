...
QUnit.module("spreadsheet > list plugin", {}, () => {
    QUnit.test("List export", async (assert) => {
        ...
        assert.strictEqual(getCellFormula(model, "A1"), `=FLECTRA.LIST.HEADER(1,"foo")`);
        assert.strictEqual(getCellFormula(model, "B1"), `=FLECTRA.LIST.HEADER(1,"bar")`);
        assert.strictEqual(getCellFormula(model, "C1"), `=FLECTRA.LIST.HEADER(1,"date")`);
        assert.strictEqual(getCellFormula(model, "D1"), `=FLECTRA.LIST.HEADER(1,"product_id")`);
        assert.strictEqual(getCellFormula(model, "A2"), `=FLECTRA.LIST(1,1,"foo")`);
        assert.strictEqual(getCellFormula(model, "B2"), `=FLECTRA.LIST(1,1,"bar")`);
        assert.strictEqual(getCellFormula(model, "C2"), `=FLECTRA.LIST(1,1,"date")`);
        assert.strictEqual(getCellFormula(model, "D2"), `=FLECTRA.LIST(1,1,"product_id")`);
        assert.strictEqual(getCellFormula(model, "A3"), `=FLECTRA.LIST(1,2,"foo")`);
        assert.strictEqual(getCellFormula(model, "A11"), `=FLECTRA.LIST(1,10,"foo")`);
    ...
    QUnit.test("Can display a field which is not in the columns", async function (assert) {
        const { model } = await createSpreadsheetWithList();
        setCellContent(model, "A1", `=FLECTRA.LIST(1,1,"active")`);
    ...
    QUnit.test("Can remove a list with undo after editing a cell", async function (assert) {
        const { model } = await createSpreadsheetWithList();
        assert.ok(getCellContent(model, "B1").startsWith("=FLECTRA.LIST.HEADER"));
    ...
    QUnit.test("Json fields are not supported in list formulas", async function (assert) {
        ...
        setCellContent(model, "A1", `=FLECTRA.LIST(1,1,"foo")`);
        setCellContent(model, "A2", `=FLECTRA.LIST(1,1,"jsonField")`);
        await waitForDataSourcesLoaded(model);
    ...
    QUnit.test("can select a List from cell formula", async function (assert) {
        ...
        model.dispatch("SELECT_FLECTRA_LIST", { listId });
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithList();
            setCellContent(model, "A1", `=3*FLECTRA.LIST("1","1","foo")`);
            const sheetId = model.getters.getActiveSheetId();
            const listId = model.getters.getListIdFromPosition({ sheetId, col: 0, row: 0 });
            model.dispatch("SELECT_FLECTRA_LIST", { listId });
            const selectedListId = model.getters.getSelectedListId();
    ...
    QUnit.test("can select a List from cell formula within a formula", async function (assert) {
        const { model } = await createSpreadsheetWithList();
        setCellContent(model, "A1", `=SUM(FLECTRA.LIST("1","1","foo"),1)`);
        const sheetId = model.getters.getActiveSheetId();
        const listId = model.getters.getListIdFromPosition({ sheetId, col: 0, row: 0 });
        model.dispatch("SELECT_FLECTRA_LIST", { listId });
        const selectedListId = model.getters.getSelectedListId();
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithList();
            setCellContent(model, "A1", `=FLECTRA.LIST(G10,"1","foo")`);
    ...
    QUnit.test("Referencing non-existing fields does not crash", async function (assert) {
        ...
        await nextTick();
        setCellContent(model, "A1", `=FLECTRA.LIST.HEADER("1", "${forbiddenFieldName}")`);
        setCellContent(model, "A2", `=FLECTRA.LIST("1","1","${forbiddenFieldName}")`);
            ...
            sheets: [
                {
                    id: "sheet1",
                    cells: {
                        A1: { content: `=FLECTRA.LIST("1", "1", "name")` },
                    },
                },
            ],
            lists: {
    ...
    QUnit.test("rename list with empty name is refused", async (assert) => {
        const { model } = await createSpreadsheetWithList();
        const result = model.dispatch("RENAME_FLECTRA_LIST", {
            listId: "1",
            name: "",
        });
    ...
    QUnit.test("Undo/Redo for RENAME_FLECTRA_LIST", async function (assert) {
        assert.expect(4);
        const { model } = await createSpreadsheetWithList();
        assert.equal(model.getters.getListName("1"), "List");
        model.dispatch("RENAME_FLECTRA_LIST", { listId: "1", name: "test" });
    ...
    QUnit.test("Can delete list", async function (assert) {
        const { model } = await createSpreadsheetWithList();
        model.dispatch("REMOVE_FLECTRA_LIST", { listId: "1" });
    ...
    QUnit.test("can edit list domain", async (assert) => {
        ...
        await waitForDataSourcesLoaded(model);
        assert.strictEqual(getCellValue(model, "B2"), "");
        const result = model.dispatch("UPDATE_FLECTRA_LIST_DOMAIN", {
            listId: "invalid",
            domain: [],
        });
        assert.deepEqual(result.reasons, [CommandResult.ListIdNotFound]);
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "A1", '=FLECTRA.LIST(1, 1, "pognon")');
            setCellContent(model, "A2", '=FLECTRA.LIST(1, 1, "currency_id")');
            await waitForDataSourcesLoaded(model);
    ...
    QUnit.test("Spec of web_search_read is minimal", async function (assert) {
        ...
        setCellContent(model, "A1", '=FLECTRA.LIST(1, 1, "pognon")');
        setCellContent(model, "A2", '=FLECTRA.LIST(1, 1, "currency_id")');
        setCellContent(model, "A3", '=FLECTRA.LIST(1, 1, "foo")');
        await waitForDataSourcesLoaded(model);
        assert.verifySteps(["web_search_read"]);
    });
    ...
    QUnit.test(
        ...
        async function (assert) {
                ...
                sheets: [
                    {
                        id: "sheet1",
                        cells: {
                            A1: { content: `=FLECTRA.LIST("1", "1", "foo")` },
                        },
                    },
                ],
                lists: {
            ...
            setCellContent(model, "A1", `=FLECTRA.LIST("1", "42", "foo")`);
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            model.dispatch("REFRESH_FLECTRA_LIST", { listId: "1" });
            await waitForDataSourcesLoaded(model);
            headerCell = getEvaluatedCell(model, "A3");
            cell = getEvaluatedCell(model, "C3");
