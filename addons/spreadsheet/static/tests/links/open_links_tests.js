...
QUnit.test("click a web link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Flectra](https://flectrahq.com)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    assert.strictEqual(urlRepresentation(cell.link, model.getters), "https://flectrahq.com");
    openLink(cell.link, env);
    assert.verifySteps(["https://flectrahq.com"]);
});
...
QUnit.test("click a menu link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[label](flectra://ir_menu_xml_id/test_menu)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
...
QUnit.test("click a menu link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: `[a view](flectra://view/${JSON.stringify(view)})` } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    assert.strictEqual(urlRepresentation(cell.link, model.getters), "an flectra view");
    openLink(cell.link, env);
