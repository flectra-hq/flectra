...
export class FlectraLineChart extends FlectraChart {
...
chartRegistry.add("flectra_line", {
    match: (type) => type === "flectra_line",
    createChart: (definition, sheetId, getters) => new FlectraLineChart(definition, sheetId, getters),
    getChartRuntime: createFlectraChartRuntime,
    validateChartDefinition: (validator, definition) =>
        FlectraLineChart.validateChartDefinition(validator, definition),
    transformDefinition: (definition) => FlectraLineChart.transformDefinition(definition),
    getChartDefinitionFromContextCreation: () => FlectraLineChart.getDefinitionFromContextCreation(),
    name: _t("Line"),
});
...
function createFlectraChartRuntime(chart, getters) {
    ...
    for (let [index, { label, data, cumulatedStart }] of datasets.entries()) {
        ...
        if (chart.stacked) {
            // use the transparency of Flectra to keep consistency
