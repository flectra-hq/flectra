...
export class PivotUIPlugin extends spreadsheet.UIPlugin {
    ...
    handleEvent(event) {
        ...
        switch (event.type) {
            case "ZonesSelected": {
                ...
                if (cell !== undefined && cell.content.startsWith("=FLECTRA.PIVOT.HEADER(")) {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "REMOVE_PIVOT":
                ...
                break;
            case "REFRESH_PIVOT":
                this._refreshFlectraPivot(cmd.id);
                break;
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshFlectraPivots();
                break;
            ...
            case "UPDATE_FLECTRA_PIVOT_DOMAIN": {
            ...
            case "REDO": {
                ...
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) => cmd.type === "UPDATE_FLECTRA_PIVOT_DOMAIN" || cmd.type === "INSERT_PIVOT"
                );
     ...
     * @param {import("@flectra/o-spreadsheet").Token[]} tokens
     */
     ...
     *
     * =FLECTRA.PIVOT(1,"expected_revenue","stage_id",2,"city","Brussels")
     * =FLECTRA.PIVOT.HEADER(1,"stage_id",2,"city","Brussels")
     * =FLECTRA.PIVOT.HEADER(1,"stage_id",2,"city","Brussels","measure","expected_revenue")
     *
     * the result is the same: ["stage_id", 2, "city", "Brussels"]
     *
     * If the cell is the result of FLECTRA.PIVOT.TABLE, the result is the domain of the cell
    ...
    getPivotDomainArgsFromPosition(position) {
        ...
        if (functionName === "FLECTRA.PIVOT.TABLE") {
            ...
            if (!cell.content.replaceAll(" ", "").toUpperCase().startsWith("=FLECTRA.PIVOT.TABLE")) {
                return undefined;
            }
        ...
        let domain = args.slice(functionName === "FLECTRA.PIVOT" ? 2 : 1);
        if (domain.at(-2) === "measure") {
            domain = domain.slice(0, -2);
        }
        const isHeader = functionName === "FLECTRA.PIVOT.HEADER";
     ...
     */
    computeFlectraPivotHeaderValue(pivotId, domainArgs) {
        const dataSource = this.getters.getPivotDataSource(pivotId);
        dataSource.markAsHeaderUsed(domainArgs);
        return dataSource.computeFlectraPivotHeaderValue(domainArgs);
    }

    /**
     * High level method computing the formatted result of FLECTRA.PIVOT.HEADER functions.
     *
    ...
    getPivotHeaderFormattedValue(pivotId, pivotArgs) {
        const dataSource = this.getters.getPivotDataSource(pivotId);
        const value = dataSource.computeFlectraPivotHeaderValue(pivotArgs);
     ...
     * @param {string} sheetId sheet id on which the formula tokens are
     * @param {import("@flectra/o-spreadsheet").Token[]} tokens Formula of the pivot cell
     *
     ...
     */
    _refreshFlectraPivots() {
        for (const pivotId of this.getters.getPivotIds()) {
            this._refreshFlectraPivot(pivotId, false);
        }
    }
...
PivotUIPlugin.getters = [
    ...
    "computeFlectraPivotHeaderValue",
