...
export const FLECTRA_VERSION = 6;

const MAP = {
    PIVOT: "FLECTRA.PIVOT",
    "PIVOT.HEADER": "FLECTRA.PIVOT.HEADER",
    "PIVOT.POSITION": "FLECTRA.PIVOT.POSITION",
    "FILTER.VALUE": "FLECTRA.FILTER.VALUE",
    LIST: "FLECTRA.LIST",
    "LIST.HEADER": "FLECTRA.LIST.HEADER",
};

const dmyRegex = /^([0|1|2|3][1-9])\/(0[1-9]|1[0-2])\/(\d{4})$/i;

export function migrate(data) {
    let _data = load(data, !!flectra.debug);
    const version = _data.flectraVersion || 0;
    if (version < 1) {
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["FLECTRA.PIVOT", "FLECTRA.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
export class FlectraVersion extends CorePlugin {
    export(data) {
        data.flectraVersion = FLECTRA_VERSION;
    }
}

FlectraVersion.getters = [];

corePluginRegistry.add("flectraMigration", FlectraVersion);
