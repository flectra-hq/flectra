...
import { Model, parse, helpers, iterateAstNodes, constants } from "@flectra/o-spreadsheet";
 ...
 * @returns {object}
 */
export async function freezeFlectraData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const figure of sheet.figures) {
            if (figure.tag === "chart" && figure.data.type.startsWith("flectra_")) {
                await loadBundle("web.chartjs_lib");
                const img = flectraChartToImage(model, figure);
                figure.tag = "image";
                figure.data = {
                    path: img,
 ...
 * @returns {boolean}
 */
function containsFlectraFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
        (!content.toUpperCase().includes("FLECTRA.") && !content.toUpperCase().includes("_T"))
    ) {
        return false;
    }
    try {
        ...
        return iterateAstNodes(ast).some(
            (ast) =>
                ast.type === "FUNCALL" &&
                (ast.value.toUpperCase().startsWith("FLECTRA.") ||
 ...
 * @returns {string}
 */
function flectraChartToImage(model, figure) {
