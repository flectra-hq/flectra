...
export class ListUIPlugin extends spreadsheet.UIPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "START":
                ...
                break;
            case "INSERT_FLECTRA_LIST": {
                const { id, linesNumber } = cmd;
                this._setupListDataSource(id, linesNumber);
                break;
            }
            case "SELECT_FLECTRA_LIST":
                this._selectList(cmd.listId);
                break;
            case "REMOVE_FLECTRA_LIST":
                if (cmd.listId === this.selectedListId) {
                    this.selectedListId = undefined;
                }
                break;
            case "REFRESH_FLECTRA_LIST":
                this._refreshFlectraList(cmd.listId);
                break;
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshFlectraLists();
                break;
            ...
            case "CLEAR_GLOBAL_FILTER_VALUE":
                this._addDomains();
                break;
            case "UPDATE_FLECTRA_LIST_DOMAIN": {
            ...
            case "REDO": {
                ...
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_FLECTRA_LIST_DOMAIN" || cmd.type === "INSERT_FLECTRA_LIST"
                );
     ...
     */
    _refreshFlectraLists() {
        for (const listId of this.getters.getListIds()) {
            this._refreshFlectraList(listId);
        }
    }
     ...
     *
     * @param {string} content Flectra list formula
     */
