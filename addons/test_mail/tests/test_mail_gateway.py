...
from flectra.addons.mail.models.mail_thread import MailThread
from flectra.addons.mail.tests.common import mail_new_test_user, MailCommon
from flectra.addons.test_mail.data import test_mail_data
from flectra.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE, THAI_EMAIL_WINDOWS_874
from flectra.addons.test_mail.models.test_mail_models import MailTestGateway, MailTestGatewayGroups, MailTestTicket
from flectra.sql_db import Cursor
from flectra.tests import tagged, RecordCapturer
...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs('flectra.addons.mail.models.mail_thread', level="WARNING") as capture:
        ...
        self.assertEqual(capture.output, [
            ("WARNING:flectra.addons.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_eml(self):
        ...
        mail = self.format(test_mail_data.MAIL_EML_ATTACHMENT, email_from='"Sylvie Lelitre" <test.sylvie.lelitre@agrolait.com>', to=f'generic@{self.alias_domain}',
                           msg_id='<cb7eaf62-58dc-2017-148c-305d0c78892f@flectrahq.com>',
...
class MailGatewayCommon(MailCommon):
    ...
    def _reinject(self, force_msg_id=False, debug_log=False):
        ...
        if mail["headers"].get("X-Flectra-Message-Id"):
            extra += f'\nX-Flectra-Message-Id: {mail["headers"]["X-Flectra-Message-Id"]}'
...
class TestMailgateway(MailGatewayCommon):
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not flectrabot:
        ...
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, flectrabot.email_formatted, f'groups@{self.alias_domain}',
        ...
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: flectrabot -> no follower')
    ...
    @mute_logger('flectra.addons.mail.models.mail_mail', 'flectra.addons.mail.models.mail_thread', 'flectra.models.unlink', 'flectra.tests')
    ...
    @mute_logger('flectra.addons.mail.models.mail_mail', 'flectra.addons.mail.models.mail_thread', 'flectra.models')
    def test_message_route_alias_owner_author_notify(self):
        ...
        self.assertFalse(self.user_root.active, 'notification logic relies on flectrabot being archived')
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'Flectrabot should be creating the message')
    ...
    @mute_logger('flectra.addons.mail.models.mail_thread', 'flectra.addons.mail.models.mail_mail', 'flectra.models', 'flectra.sql_db')
    def test_message_process_alias_config_invalid_defaults(self):
        ...
        with self.mock_mail_gateway(), patch('flectra.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
        ...
        with self.assertRaises(Exception), patch('flectra.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
                                                 autospec=True) as _alias_bounce_incoming_email_mock:
    ...
    @mute_logger('flectra.addons.mail.models.mail_thread', 'flectra.models', 'flectra.tests')
    ...
    @mute_logger('flectra.addons.mail.models.mail_thread', 'flectra.models.unlink')
    ...
    @mute_logger('flectra.addons.mail.models.mail_thread', 'flectra.models.unlink', 'flectra.addons.mail.models.mail_mail', 'flectra.tests')
    def test_message_process_external_notification_reply(self):
        ...
        self.assertFalse(new_msg.is_internal, "Responses to messages sent by flectrabot should always be public.")
        ...
        new_msg = self.test_record.message_ids[0]
        self.assertTrue(new_msg.is_internal, "Responses to messages sent by anyone but flectrabot should keep"
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Flectra is still capable of decoding it.
...
class TestMailGatewayLoops(MailGatewayCommon):
    ...
    def test_routing_loop_auto_notif(self):
        """ Test Flectra servers talking to each other """
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.other_partner.email_formatted,
    ...
    def test_routing_loop_follower_alias(self):
        ...
        # simulate this email coming back to the same Flectra server -> msg_id is
...
class TestMailGatewayReplies(MailGatewayCommon):
    ...
    def test_routing_reply_incoming_email(self):
        ...
        self.assertMailNotifications(
            reply,
            [{
                'content': 'Flectra Reply',
                'email_values': {
                    'message_id': reply.message_id,
                    'references': f'{email.message_id} {reply.message_id}',  # should contain reference to FlectraExternal message
                },
                'fields_values': {
    ...
    def test_routing_reply_internal_messages(self):
        """ Test routing notably between two Flectras when internal messages
        ...
        Action                  Flectra1                   Flectra2
        RFQ-like                                        creation log
                                                        initial_msg
        Flectra2 replies           creation log
                                reply                   reply
        -some internal work-                            user_notification
        Flectra1 replies           reply_2                 reply_2 (incoming email)
        -some internal work-                            log
        Flectra2 replies           reply_3                 reply_3 (outgoing email)

        Purpose: have references from Flectra2 containing message IDs to try to
        ...
        gateway_record.message_post(
            author_id=self.env.ref('base.partner_root').id,
            body='FlectraExternal Inquiry',
            email_from=self.partner_1.email_normalized,
            message_type='comment',
            subtype_id=self.env.ref('mail.mt_comment').id,
        )
        self.assertEqual(gateway_record.message_partner_ids, self.partner_admin)
        log, flectraext_msg = gateway_record.message_ids[1], gateway_record.message_ids[0]
        self.assertEqual(flectraext_msg.parent_id, log, 'Log serves as thread ancestor')
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    'message_id': reply.message_id,
                    'references': f'{flectraext_msg.message_id} {reply.message_id}',  # should contain reference to FlectraExternal message
                },
                'fields_values': {
        ...
        # coming from Flectra1: their reply as an incoming email
        with self.mock_mail_gateway():
            self.format_and_process(
                MAIL_TEMPLATE, self.email_from, reply.reply_to,
                subject='Gateway Creation',
                extra=f'References: {reply.message_id} <msg1@flectra1>',
                debug_log=True,
            )
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{flectraext_msg.message_id} {reply.message_id} {reply_2.message_id}',  # should contain reference to FlectraExternal message
                },
                'fields_values': {
        ...
        self.assertMailNotifications(
            reply_3,
            [{
                'content': 'Flectra Reply 2',
                'email_values': {
                    'message_id': reply_3.message_id,
                    'references': f'{flectraext_msg.message_id} {reply.message_id} {reply_2.message_id} {reply_3.message_id}',  # should contain reference to FlectraExternal message
                },
                'fields_values': {
                    'notified_partner_ids': self.partner_1 + self.partner_admin,
                    'parent_id': log,  # log serves as thread ancestor
