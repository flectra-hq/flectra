...
import { markRaw, reactive } from "@flectra/owl";
...
export class PosStore extends Reactive {
    ...
    _loadPosSession() {
        ...
        this.pos_session.login_number = flectra.login_number;
    }
    ...
    async load_new_partners() {
        ...
        const partners = await this.orm.silent.call(
            ...
            [[flectra.pos_session_id], search_params]
        );
    ...
    async _loadMissingProducts(orders) {
        ...
        const products = await this.orm.call(
            ...
            [flectra.pos_session_id, { domain: [["id", "in", [...missingProductIds]]] }]
        );
    ...
    async _loadMissingPricelistItems(products) {
        ...
        const pricelistItems = await this.orm.call(
            ...
            [flectra.pos_session_id, product_tmpl_ids, product_ids]
        );
    ...
    async _loadPartners(partnerIds) {
        if (partnerIds.length > 0) {
            ...
            const fetchedPartners = await this.orm.silent.call(
                "pos.session",
                "get_pos_ui_res_partner_by_params",
                [[flectra.pos_session_id], { domain: [["id", "in", partnerIds]] }]
            );
    ...
    async _getPricelistJson(pricelistsToGet) {
        return await this.env.services.orm.call(
            ...
            [[flectra.pos_session_id], pricelistsToGet]
        );
    }
    ...
    async _getFiscalPositionJson(fiscalPositionToGet) {
        return await this.env.services.orm.call(
            ...
            [[flectra.pos_session_id], fiscalPositionToGet]
        );
    }
        ...
        if (setAvailable) {
            try {
                ...
                const ignoreError =
                    ...
                    error.exceptionName === "flectra.exceptions.AccessError";
                if (!ignoreError) {
                    throw error;
                }
            }
