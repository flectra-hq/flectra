...
import * as FlectraEditorLib from "@web_editor/js/editor/flectra-editor/src/FlectraEditor";
...
import { isSelectionInSelectors, peek } from '@web_editor/js/editor/flectra-editor/src/utils/utils';
...
const FlectraEditor = FlectraEditorLib.FlectraEditor;
const getDeepRange = FlectraEditorLib.getDeepRange;
const getInSelection = FlectraEditorLib.getInSelection;
const isProtected = FlectraEditorLib.isProtected;
const rgbToHex = FlectraEditorLib.rgbToHex;
const preserveCursor = FlectraEditorLib.preserveCursor;
const closestElement = FlectraEditorLib.closestElement;
const setSelection = FlectraEditorLib.setSelection;
const endPos = FlectraEditorLib.endPos;
const hasValidSelection = FlectraEditorLib.hasValidSelection;
const parseHTML = FlectraEditorLib.parseHTML;
const closestBlock = FlectraEditorLib.closestBlock;
const getRangePosition = FlectraEditorLib.getRangePosition;
const fillEmpty = FlectraEditorLib.fillEmpty;
const isVisible = FlectraEditorLib.isVisible;
const getSelectedNodes = FlectraEditorLib.getSelectedNodes;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.flectraEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.flectraEditor.historyUnstash();
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.flectraEditor.withoutRollback(() => this.flectraEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        this.flectraEditor.addEventListener('contentChanged', function () {
        ...
        if (this.flectraEditor.document !== document) {
            this.flectraEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.flectraEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.flectraEditor.document.addEventListener("keyup", this._signalOnline, true);
        }

        this._initialValue = this.getValue();
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.flectraEditor.multiselectionRefresh, { passive: true });
        ...
        this._observeFlectraFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.flectraEditor.observerUnactive();
                const $field = $(this);
                ...
                self.flectraEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.flectraEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            $(this.flectraEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.flectraEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.flectraEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.flectraEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.flectraEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.flectraEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        this.setCSSVariables(this.snippetsMenu ? this.snippetsMenu.el : this.toolbarEl);

        this.flectraEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.flectraEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.flectraEditor.isMobile) {
                this.flectraEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.flectraEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.flectraEditor) {
            this.flectraEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.flectraEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.flectraEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.flectraEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.flectraEditor.destroy();
        }
        if (this.snippetsMenu) {
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length && this.flectraEditor) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.flectraEditor.multiselectionRefresh, { passive: true });
        }
        $(this.$root).off('click');
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper flectra-editor">');
            this.$root = $wrapper;
    ...
    getValue(options) {
        ...
        if (this.flectraEditor) {
            this.flectraEditor.cleanForSave($editable[0]);
     ...
     */
    historyReset() {
        this.flectraEditor.historyReset();
    }
    /**
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.flectraEditor) : [];
     ...
     */
    setValue(value) {
        this.flectraEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.flectraEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.flectraEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.flectraEditor && !this.flectraEditor.historyResetLatestComputedSelection(true)) {
    ...
    getDeepRange() {
        return getDeepRange(this.flectraEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.flectraEditor.editable) {
        if (this.flectraEditor) {
            this.flectraEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.flectraEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the Flectra field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeFlectraFieldChanges() {
        ...
        if (this.flectraFieldObservers) {
            for (let observerData of this.flectraFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const flectraFieldSelector = '[data-oe-model], [data-oe-translation-initial-sha]';
            const $flectraFields = this.$editable.find(flectraFieldSelector);
            const renderingClassesSelector = this.flectraEditor.options.renderingClasses
                .map(className => `.${className}`).join(", ");
            this.flectraFieldObservers = [];

            $flectraFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.flectraEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $flectraFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    //  `_pauseFlectraFieldObservers(field)` call.
                    this.__flectraFieldObserversToPause = this.flectraFieldObservers.filter(
                    ...
                    this._pauseFlectraFieldObservers();
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.flectraEditor.options.renderingClasses);
                    }
                    const html = $(fieldNodeClone).html();
                    this.flectraEditor.withoutRollback(() => {
                ...
                this.flectraFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
     ...
     */
    _pauseFlectraFieldObservers() {
        // TODO adapt in master: remove this and directly exclude observers with
        // targets inside the current field (we use `this.flectraFieldObservers`
        // as fallback for compatibility here).
        const fieldObserversData = this.__flectraFieldObserversToPause || this.flectraFieldObservers;
    ...
    openLinkToolsFromSelection() {
        ...
        for (const el of getSelectedNodes(this.flectraEditor.editable)) {
            if (
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.flectraEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.flectraEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.flectraEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.flectraEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.flectraEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.flectraEditor.editable,
                        link,
                ...
                const _onClick = ev => {
                    ...
                    if (
                        ...
                        this.destroyLinkTools();
                        this.flectraEditor.document.removeEventListener('click', _onClick, true);
                        document.removeEventListener('click', _onClick, true);
                    }
                };
                this.flectraEditor.document.addEventListener('click', _onClick, true);
                document.addEventListener('click', _onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.flectraEditor.historySize() - 1;
            this.flectraEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.flectraEditor.historyUnpauseSteps();
                    this.flectraEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt') {
        const restore = preserveCursor(this.flectraEditor.document);
        const params = {
            insert: content => {
                this.flectraEditor.historyPauseSteps();
                const insertedNodes = this.flectraEditor.execCommand('insert', content);
                this.flectraEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.flectraEditor.editable.parentElement;
        ...
        if (mode === 'alternatives') {
            params.originalText = this.flectraEditor.document.getSelection().toString() || '';
        }
        this.flectraEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.snippetsMenu._mutex.exec(() => {
                // Wait for the editor panel to be fully updated.
                this.flectraEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.flectraEditor.execCommand('unlink');
        }
    }
    /**
    ...
    showImageFullscreen(url) {
        ...
        this.flectraEditor.document.getSelection()?.collapseToEnd();
        this.flectraEditor.editable.blur();
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.flectraEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.flectraEditor.document);

        const editable = FlectraEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.flectraEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.flectraEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.flectraEditor.execCommand('insert', str);
                },
            }, {
    ...
    getInSelection(selector) {
        return getInSelection(this.flectraEditor.document, selector);
    }
    /**
    ...
    async _createSnippetsMenuInstance(options={}) {
        const snippetsEditor = await flectra.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.flectraEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.flectraEditor.document});
            const destroyTransfo = () => {
                $image.transfo('destroy');
                $(this.flectraEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                this.flectraEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
                imgTransformBtn.classList.toggle('active', $image[0].matches('[style*="transform"]'));
            };
            $(this.flectraEditor.document).on('mouseup', mouseup);
            ...
            $(this.flectraEditor.document).on('mousedown', mousedown);
            ...
            this.flectraEditor.document.addEventListener('keydown', keydown);
        });
    ...
    onColorpaletteDropdownShow(colorType) {
        ...
        this.flectraEditor.historyStash();
    }
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.flectraEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.flectraEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.flectraEditor.document.contains(element));
        ...
        if (selectedTds.length === 1 && !previewMode) {
            ...
            sel.addRange(getDeepRange(this.flectraEditor.editable, { range }));
        }

        const hexColor = this._colorToHex(color);
        this.flectraEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.flectraEditor.document.getSelection();
        if (!selection) return;
        const anchorNode = selection.anchorNode;
        if (isProtected(anchorNode)) {
            return;
        }
        if (this.flectraEditor.document.querySelector(".transfo-container")) {
            return;
        }

        this.flectraEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.flectraEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.flectraEditor.document.getSelection();
            const range = this.flectraEditor.document.createRange();
        ...
        this.flectraEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent FlectraEditor from controlling it as this is
    ...
    _getBannerCommand(title, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.flectraEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.flectraEditor.document, `
                ...
                `).childNodes[0];
                this.flectraEditor.execCommand('insert', bannerElement);
                this.flectraEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...
            this._getBannerCommand(_t('Banner Danger'), 'danger', 'fa-exclamation-circle', _t('Insert a danger banner'), 21),
            {
                ...
                callback: () => {
                    this.flectraEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.flectraEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.flectraEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (user && user.signature) {
                        this.flectraEditor.execCommand('insert', parseHTML(this.flectraEditor.document, user.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.flectraEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.flectraEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.flectraEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.flectraEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.flectraEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.flectraEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.flectraEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.flectraEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.flectraEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from flectra field value
        // ZeroWidthSpace may be present from FlectraEditor edition process
    ...
    _onSelectionChange() {
        if (this.linkPopover && this.isSelectionInEditable()) {
            const selectionInLink = getInSelection(this.flectraEditor.document, 'a') === this.linkPopover.target;
            const isVisible = this.linkPopover.el.offsetParent;
            if (
                isVisible &&
                (
                    (this.options.autohideToolbar && !this.flectraEditor.document.getSelection().isCollapsed) ||
                    !selectionInLink
                )
            ) {
                this.linkPopover.hide();
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.flectraEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.flectraEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.flectraEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.flectraEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.flectraEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.flectraEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.flectraEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.flectraEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.flectraEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.flectraEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.flectraEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.flectraEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.flectraEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.flectraEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.flectraEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the flectra bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.flectraEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.flectraEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.flectraEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.flectraEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.flectraEditor.historyResetFromSteps(steps, historyIds);
        this.flectraEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.flectraEditor && !this.flectraEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.flectraEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.flectraEditor.historyGetBranchIds().join(',');
    ...
    _beforeAnyCommand() {
        ...
        for (const node of FlectraEditorLib.getTraversedNodes(this.$editable[0])) {
