...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/flectra-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.flectraEditor) {
            const selection = this.wysiwyg.flectraEditor.document.getSelection();
    ...
    async startWysiwyg(wysiwyg) {
        ...
        wysiwyg.$editable[0].classList.add("flectra-editor-qweb");
        ...
        this.wysiwyg.flectraEditor.addEventListener("historyStep", () =>
            this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
        );

        if (this.props.isCollaborative) {
            this.wysiwyg.flectraEditor.addEventListener("onExternalHistorySteps", () =>
                this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
            );
        }
    ...
    toggleCodeView() {
        ...
        if (this.wysiwyg) {
            this.wysiwyg.flectraEditor.observerUnactive('toggleCodeView');
            if (this.state.showCodeView) {
                this.wysiwyg.$editable.remove();
                this.wysiwyg.flectraEditor.toolbarHide();
                const value = this.wysiwyg.getValue();
                this.props.record.update({ [this.props.name]: value });
            } else {
                this.wysiwyg.flectraEditor.observerActive('toggleCodeView');
            }
        }
        if (!this.state.showCodeView) {
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.flectraEditor.execCommand('insert', t);
            // Ensure the dynamic placeholder <t> element is sanitized.
            this.wysiwyg.flectraEditor.sanitize(t);
        }
    }
    ...
    async commitChanges({ urgent, shouldInline } = {}) {
        if (this._isDirty() || urgent || (shouldInline && this.props.isInlineStyle)) {
            ...
            if (this.wysiwyg && this.wysiwyg.flectraEditor) {
                this.wysiwyg.flectraEditor.observerUnactive('commitChanges');
                ...
                this.wysiwyg.flectraEditor.observerActive('commitChanges');
            }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await flectra.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
        this.MoveNodePlugin = (await flectra.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin'))?.MoveNodePlugin;
        // Otherwise, load the module.
        if (!wysiwygModule) {
            await loadBundle('web_editor.backend_assets_wysiwyg');
            wysiwygModule = await flectra.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
            this.MoveNodePlugin = (await flectra.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin')).MoveNodePlugin;
        }
    ...
    async _toInline() {
        ...
        this.wysiwyg.flectraEditor.sanitize(this.wysiwyg.flectraEditor.editable);
        const html = this.wysiwyg.getValue();
        const $flectraEditor = $editable.closest('.flectra-editor-editable');
        // Save correct nodes references.
        // Remove temporarily the class so that css editing will not be converted.
        $flectraEditor.removeClass('flectra-editor-editable');
        $editable.html(html);

        await toInline($editable, undefined, this.wysiwyg.$iframe);
        $flectraEditor.addClass('flectra-editor-editable');
...
export const htmlField = {
    ...
    extractProps({ attrs, options }, dynamicInfo) {
        ...
        return {
            codeview: Boolean(flectra.debug && options.codeview),
            placeholder: attrs.placeholder,
