...
import { closestElement, isUnremovable } from "@web_editor/js/editor/flectra-editor/src/utils/utils";
...
var SnippetEditor = Widget.extend({
    ...
    start: function () {
        ...
        if (this.isTargetMovable) {
            ...
            if (!this.$target[0].matches("section")) {
                ...
                this.options.wysiwyg.flectraEditor.observerUnactive('image_drag_and_drop');
                this.$target[0].classList.add("o_draggable");
                this.draggableComponentImgs = this._initDragAndDrop("img", ".o_draggable", this.$target[0]);
                this.options.wysiwyg.flectraEditor.observerActive('image_drag_and_drop');
            }
        } else {
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.flectraEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.flectraEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.flectraEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart({ helper, addStyle }) {
        this.options.wysiwyg.flectraEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.flectraEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
        ...
        if (rowEl.classList.contains('row') && this.options.isWebsite) {
            if (allowGridMode) {
                ...
                if (!rowEl.classList.contains('o_grid_mode')) {
                    this.options.wysiwyg.flectraEditor.observerActive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStop({ x, y }) {
        this.options.wysiwyg.flectraEditor.automaticStepActive();
        this.options.wysiwyg.flectraEditor.automaticStepSkipStack();
        this.options.wysiwyg.flectraEditor.unbreakableStepUnactive();
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        $(this.options.wysiwyg.flectraEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.flectraEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.flectraEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            ...
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.flectraEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.flectraEditor.historyCanRedo());
                };
                this.options.wysiwyg.flectraEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.flectraEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.flectraEditor.editable).trigger('content_changed');
                });
            }
    ...
    _makeSnippetDraggable() {
        ...
        const dragAndDropOptions = this.options.getDragAndDropOptions({
            ...
            onDragStart: ({ element }) => {
                ...
                const doc = this.options.wysiwyg.flectraEditor.document;
                $(doc.body).addClass('oe_dropzone_active');

                this.options.wysiwyg.flectraEditor.automaticStepUnactive();

                this.$el.find('.oe_snippet_thumbnail').addClass('o_we_already_dragging');
                this.options.wysiwyg.flectraEditor.observerUnactive('dragAndDropCreateSnippet');
            ...
            onDragEnd: async ({ x, y, helper }) => {
                ...
                this.options.wysiwyg.flectraEditor.observerActive('dragAndDropCreateSnippet');
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _checkEditorToolbarVisibility: function (e) {
        ...
        const selection = this.options.wysiwyg.flectraEditor.document.getSelection();
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.flectraEditor.editable));
    },
    /**
