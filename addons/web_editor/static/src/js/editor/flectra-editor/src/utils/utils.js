...
export function closestElement(node, predicate = "*") {
    ...
    return element?.closest('.flectra-editor-editable') && element;
}
...
export function getNormalizedCursorPosition(node, offset, full = true) {
    const editable = closestElement(node, '.flectra-editor-editable');
...
// TODO rename this function in master: it also handles Flectra icons, not only
...
export function prepareUpdate(...args) {
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, '.flectra-editor-editable');
