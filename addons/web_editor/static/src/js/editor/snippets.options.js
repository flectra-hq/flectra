...
const resetOuids = FlectraEditorLib.resetOuids;
let _serviceCache = {
    orm: {},
    rpc: {},
};
...
const SnippetOptionWidget = Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.flectraEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.flectraEditor.automaticStepActive('preview_option');
        }
    ...
    _onUserValueUpdate: async function (ev) {
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.flectraEditor.unbreakableStepUnactive();
        }
        this.trigger_up('snippet_edition_request', {exec: async () => {
            ...
            if (shouldRecordUndo) {
                this.options.wysiwyg.flectraEditor.historyStep();
            }
...
registry.sizing = SnippetOptionWidget.extend({
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ...
            self.options.wysiwyg.flectraEditor.automaticStepUnactive('resizing');
            ...
            const bodyMouseUp = function () {
                ...
                self.options.wysiwyg.flectraEditor.automaticStepActive('resizing');
                ...
                setTimeout(function () {
                    self.options.wysiwyg.flectraEditor.historyStep();
...
registry.layout_column = SnippetOptionWidget.extend(ColumnLayoutMixin, {
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["row-gap", "column-gap"].includes(params.cssProperty) && !isMobileView) {
            ...
            this.options.wysiwyg.flectraEditor.observerUnactive("addGridPreview");
            ...
            this.options.wysiwyg.flectraEditor.observerActive("addGridPreview");
     ...
     */
    _removeGridPreview() {
        this.options.wysiwyg.flectraEditor.observerUnactive("removeGridPreview");
        ...
        this.options.wysiwyg.flectraEditor.observerActive("removeGridPreview");
    },
    /**
     * @returns {boolean}
     */
...
registry.GridColumns = SnippetOptionWidget.extend({
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["--grid-item-padding-y", "--grid-item-padding-x"].includes(params.cssProperty)) {
            ...
            this.options.wysiwyg.flectraEditor.observerUnactive("addPaddingPreview");
            this.$target[0].classList.add("o_we_padding_highlight");
            this.options.wysiwyg.flectraEditor.observerActive("addPaddingPreview");
    ...
    _removePaddingPreview() {
        this.options.wysiwyg.flectraEditor.observerUnactive("removePaddingPreview");
        ...
        this.options.wysiwyg.flectraEditor.observerActive("removePaddingPreview");
    },
});
...
registry.ReplaceMedia = SnippetOptionWidget.extend({
     ...
     */
    onFocus() {
        this.options.wysiwyg.flectraEditor.addEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.flectraEditor.addEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
     ...
     */
    onBlur() {
        this.options.wysiwyg.flectraEditor.removeEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.flectraEditor.removeEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
    },
...
registry.ImageTools = ImageHandlerOption.extend({
    ...
    async setImgShapeHoverEffect(previewMode, widgetValue, params) {
        ...
        if (this.firstHoverEffect) {
            this.options.wysiwyg.flectraEditor.historyUnpauseSteps();
...
￼    */
    ...
    async _writeShape(svgText) {
        ...
        if (hasHoverEffect) {
            ...
            this.options.wysiwyg.flectraEditor.observerUnactive("addClonedImgForHoverEffectPreview");
            img.classList.add("d-none");
            img.insertAdjacentElement("afterend", clonedImgEl);
            this.options.wysiwyg.flectraEditor.observerActive("addClonedImgForHoverEffectPreview");
        }
        const loadedImg = await loadImage(dataURL, img);
        if (hasHoverEffect) {
            this.options.wysiwyg.flectraEditor.observerUnactive("removeClonedImgForHoverEffectPreview");
            clonedImgEl.remove();
            img.classList.remove("d-none");
            this.options.wysiwyg.flectraEditor.observerActive("removeClonedImgForHoverEffectPreview");
        }
...
registry.BackgroundImage = SnippetOptionWidget.extend({
    ...
    _setBackground(backgroundURL) {
        ...
        const combined = backgroundImagePartsToCss(parts);
        this.$target.css('background-image', combined);
        this.options.wysiwyg.flectraEditor.editable.focus();
    },
});
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.flectraEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.flectraEditor.observerActive('_markColorLevel');
    },
});
...
registry.VersionControl = SnippetOptionWidget.extend({
    ...
    async replaceSnippet() {
        ...
        // Replacing the block.
        this.options.wysiwyg.flectraEditor.historyPauseSteps();
