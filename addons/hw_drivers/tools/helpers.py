...
from flectra import http, release, service
...
def check_git_branch():
    ...
    try:
        ...
        if response.status == 200:
            ...
            _logger.info(
                "Current IoT Box local git branch: %s / Associated Flectra database's git branch: %s",
                local_branch,
                db_branch,
            )
            ...
            if db_branch != local_branch:
                try:
                    with writable():
                        ...
                        _logger.info("Updating flectra folder to the branch %s", db_branch)
                        subprocess.run(
                            ['/home/pi/flectra/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh'], check=True
                        )
...
def save_conf_server(url, token, db_uuid, enterprise_code):
    """
    Save server configurations in flectra.conf
...
def load_certificate():
    ...
    if error or not response.get('data') or not json.loads(response.data.decode('utf8')).get('result'):
        _logger.warning("An error received from flectrahq.com while trying to get the certificate: %s", error or 'Empty response')
...
def delete_iot_handlers():
    ...
    try:
        ...
        filenames = [
            f"flectra/addons/hw_drivers/iot_handlers/{file.relative_to(iot_handlers)}"
    ...
    if server:
        ...
        try:
            ...
            if resp.data:
                delete_iot_handlers()
                with writable():
                    path = path_file('flectra', 'addons', 'hw_drivers', 'iot_handlers')
...
def path_file(*args):
    """Return the path to the file from IoT Box root or Windows Flectra
    server folder
    :return: The path to the file
    """
    platform_os = platform.system()
    if platform_os == 'Linux':
        return Path("~pi", *args).expanduser() # Path.home() returns flectra user's home instead of pi's
    elif platform_os == 'Windows':
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\Flectra\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\Flectra\\downloaded_file.zip', 'C:\\Program Files\\Flectra\\new_folder'))
...
def update_conf(values, section='iot.box'):
    """
    Update flectra.conf with the given key and value.
    ...
    """
    _logger.debug("Updating flectra.conf with values: %s", values)
    conf = get_conf()
    get_conf.cache_clear()  # Clear the cache to get the updated config

    if not conf.has_section(section):
        _logger.debug("Creating new section '%s' in flectra.conf", section)
        conf.add_section(section)

    for key, value in values.items():
        conf.set(section, key, value) if value else conf.remove_option(section, key)

    write_file("flectra.conf", conf)


@cache
def get_conf(key=None, section='iot.box'):
    """
    Get the value of the given key from flectra.conf, or the full config if no key is provided.
    ...
    conf.read(path_file("flectra.conf"))
...
def disconnect_from_server():
    ...
    get_flectra_server_url.cache_clear()
    update_conf({
        'remote_server': '',
        'token': '',
        'db_uuid': '',
...
def migrate_old_config_files_to_new_config_file():
    """Migrate old config files to the new flectra.conf"""
    if not get_conf().has_section('iot.box'):
        _logger.info('Migrating old config files to the new flectra.conf')
        iotbox_version = read_file_first_line('/var/flectra/iotbox_version')
        ...
        unlink_file('flectra-db-uuid.conf')
