...
from flectra import http, service, tools
...
class IoTboxHomepage(Home):
    ...
    def save_credential(self, db_uuid, enterprise_code):
        helpers.write_file('flectra-db-uuid.conf', db_uuid)
        helpers.write_file('flectra-enterprise-code.conf', enterprise_code)
        helpers.flectra_restart(0)
    ...
    def connect_to_server(self, token, iotname):
        ...
        helpers.flectra_restart(1)  # 1 sec delay for IO operations (save_conf_server)
    ...
    @http.route('/iot_restart_flectra_or_reboot', type='json', auth='none', cors='*', csrf=False)
    def iot_restart_flectra_or_reboot(self, action):
        """ Reboots the IoT Box / restarts Flectra on it depending on chosen 'action' argument"""
        try:
            if action == 'restart_flectra':
