...
export function usePartnerAutocomplete() {
    const keepLastFlectra = new KeepLast();
    ...
    async function autocomplete(value) {
        ...
        let flectraSuggestions = [];
        let clearbitSuggestions = [];
        return new Promise((resolve, reject) => {
            const flectraPromise = getFlectraSuggestions(value, isVAT).then((suggestions) => {
            ...
            const concatResults = () => {
                ...
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    const websites = flectraSuggestions.map((suggestion) => {
                ...
                flectraSuggestions = flectraSuggestions.filter((suggestion) => {
                    return !suggestion.ignored;
                });
                flectraSuggestions.forEach((suggestion) => {
                    delete suggestion.ignored;
                });
                return resolve(flectraSuggestions);
            };

            whenAll([flectraPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    }
     ...
     * @private
     */
    async function getFlectraSuggestions(value, isVAT) {
        ...
        const suggestions = await keepLastFlectra.add(prom);
