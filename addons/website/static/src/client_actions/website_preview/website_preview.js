...
export class WebsitePreview extends Component {
    setup() {
        ...
        onWillStart(async () => {
            ...
            if (!session.website_bypass_domain_redirect // Used by the Flectra support (bugs to be expected)
                    && this.websiteDomain
        ...
        onMounted(() => {
            ...
            // FlectraFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('FlectraFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        });
        ...
        /**
         * This removes the 'Flectra' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('FlectraFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('FlectraFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.websiteService.websiteRootInstance = undefined;
            this.iframe.el.addEventListener('FlectraFrameContentLoaded', resolve, { once: true });
            if (url) {
