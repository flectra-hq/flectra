...
import { Component, markup, useRef, useState } from "@flectra/owl";
...
options.registry.OptionsTab = options.Class.extend({
    ...
    async updateUI() {
        ...
        // If the gray palette has been generated by Flectra standard option,
        ...
        // same hue by the Flectra editor, there might be rounding errors
                ...
                this.trigger_up('menu_dialog', {
                    ...
                    save: (name, url) => {
                        ...
                        return this.orm.call(
                            ...
                            widget.wysiwyg.flectraEditor.observerUnactive();
                            widget.$target.attr('href', url);
                            $menu.text(name);
                            widget.wysiwyg.flectraEditor.observerActive();
                        });
                    },
                });
                widget.popover.hide();
...
options.registry.DeviceVisibility = options.Class.extend({
     ...
     */
    async onTargetHide() {
        this.options.wysiwyg.flectraEditor.observerUnactive("onTargetHide");
        this.$target[0].classList.remove('o_snippet_override_invisible');
        this.options.wysiwyg.flectraEditor.observerActive("onTargetHide");
    },
    /**
     * @override
     */
    async onTargetShow() {
        ...
        if ((this.$target[0].classList.contains('o_snippet_mobile_invisible')
            ...
            this.options.wysiwyg.flectraEditor.observerUnactive("onTargetShow");
            this.$target[0].classList.add('o_snippet_override_invisible');
            this.options.wysiwyg.flectraEditor.observerActive("onTargetShow");
        }
    },
    /**
     * @override
...
options.registry.TextHighlight = options.Class.extend({
    ...
    */
    ...
    _autoAdaptHighlights() {
        this.trigger_up("snippet_edition_request", { exec: async () =>
            await this._refreshPublicWidgets($(this.options.wysiwyg.flectraEditor.editable))
        });
    },
