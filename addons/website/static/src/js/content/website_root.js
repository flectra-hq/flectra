...
import { markup } from "@flectra/owl";
    ...
    async _loadGMapAPI(editableMode, refetch) {
        ...
        if (refetch || !this._gmapAPILoading) {
            this._gmapAPILoading = new Promise(async resolve => {
                ...
                await loadJS(`https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=flectra_gmap_api_post_load&key=${encodeURIComponent(key)}`);
            });
        }
