...
import { isMediaElement } from '@web_editor/js/editor/flectra-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@flectra/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.flectraEditor.options.filterMutationRecords;
        Object.assign(this.flectraEditor.options, {
            /**
             * @override
             */
        ...
        // Disable FlectraEditor observer's while setting up classes
        this.flectraEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with flectraEditor
    ...
    async destroy() {
        ...
        const mod = await flectra.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await flectra.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.flectraEditor.filterMutationRecords(records);
        ...
        this.flectraEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.flectraEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by FlectraEditor when text is entered.
    ...
    async _createSnippetsMenuInstance(options = {}) {
        const snippetsEditor = await flectra.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _toggleMegaMenu(toggleEl) {
        ...
        this.flectraEditor.observerUnactive("toggleMegaMenu");
        megaMenuEl.classList.add('o_no_parent_editor');
        this.flectraEditor.observerActive("toggleMegaMenu");
