...
class TestWebsiteAssets(flectra.tests.HttpCase):
...
class TestWebAssets(flectra.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with flectra.tools.mute_logger('flectra.addons.web.controllers.binary'):
            self.assertEqual(
