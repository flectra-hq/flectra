    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        if self._is_invoice_report(report_ref):
            ...
            if qr_inv_ids:
                ...
                if header:
                    ...
                    for invoice_id, stream in qr_res.items():
                        qr_pdf = FlectraPdfFileReader(stream['stream'], strict=False)
                        header_pdf = FlectraPdfFileReader(header_res[invoice_id]['stream'], strict=False)

                        page = header_pdf.getPage(0)
                        page.mergePage(qr_pdf.getPage(0))

                        output_pdf = FlectraPdfFileWriter()
            ...
            for invoice_id, additional_stream in streams_to_append.items():
                ...
                writer.appendPagesFromReader(FlectraPdfFileReader(invoice_stream, strict=False))
                writer.appendPagesFromReader(FlectraPdfFileReader(additional_stream['stream'], strict=False))
                new_pdf_stream = io.BytesIO()
