...
class ProcurementRule(models.Model):
    ...
    def _get_stock_move_values(self, product_id, product_qty, product_uom, location_id, name, origin, values, group_id):
        ...
        :rtype: dictionary
        '''
        date_expected = (datetime.strptime(values['date_planned'], DEFAULT_SERVER_DATETIME_FORMAT) - relativedelta(days=self.delay or 0)).strftime(
    ...
    def _log_next_activity(self, product_id, note):
        existing_activity = self.env['mail.activity'].search(
                [('res_id', '=', product_id.product_tmpl_id.id), ('res_model_id', '=', self.env.ref('product.model_product_template').id),
                 ('note', '=', note)])
...
class ProcurementGroup(models.Model):
    ...
    def run(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        ...
        if not rule:
            raise UserError(_('No procurement rule found for product %s. Please verify the configuration of your routes') % product_id.name)
        ...
        for move in exception_moves:
            ...
            try:
                with self._cr.savepoint():
                    ...
                    self.run(move.product_id, move.product_uom_qty, move.product_uom, move.location_id, move.rule_id and move.rule_id.name or "/",
                             origin, values)
                        ...
                        try:
                            ...
                            if float_compare(op_product_virtual, orderpoint.product_min_qty, precision_rounding=orderpoint.product_uom.rounding) <= 0:
                                ...
                                if qty_rounded > 0:
                                    ...
                                    try:
                                        with self._cr.savepoint():
                                            self.env['procurement.group'].run(orderpoint.product_id, qty_rounded, orderpoint.product_uom,
