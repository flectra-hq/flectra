...
class AcquirerFlectraByAdyen(models.Model):
    ...
    @api.model
    def _flectra_adyen_format_amount(self, amount, currency_id):
        return {
    ...
    @api.model
    def _flectra_adyen_compute_signature(self, amount, currency_id, reference):
    ...
    def flectra_adyen_form_generate_values(self, values):
    ...
    def flectra_adyen_get_form_action_url(self):
        self.ensure_one()
...
class TxFlectraByAdyen(models.Model):
    ...
    def _flectra_adyen_form_get_invalid_parameters(self, data):
        invalid_parameters = []
    ...
    def _flectra_adyen_form_validate(self, data):
        merchant_signature = self.acquirer_id._flectra_adyen_compute_signature(self.amount, self.currency_id, self.reference)
        ...
        if self.partner_id and not self.payment_token_id and \
            ...
            token_id = self.env['payment.token'].create({
                ...
                'flectra_adyen_payment_method_type': next(pm['type'] for pm in stored_payment_methods if pm['id'] == pm_id)
            })
...
class PaymentToken(models.Model):
    ...
    flectra_adyen_payment_method_type = fields.Char(string='PaymentMethod Type')
