...
from flectra.http import root, Request, Response, SessionExpiredException, get_default_session
from flectra.modules.registry import Registry
from flectra.service import model as service_model
from flectra.service.server import CommonServer
from flectra.service.security import check_session
...
def acquire_cursor(db):
    ...
    for tryno in range(1, MAX_TRY_ON_POOL_ERROR + 1):
        with suppress(PoolError):
            return flectra.registry(db).cursor()
...
class Websocket:
    ...
    def __init__(self, sock, session):
        ...
        self.__selector = (
            ...
            if flectra.evented and hasattr(selectors, 'PollSelector')
...
class WebsocketConnectionHandler:
    ...
    def _handle_public_configuration(cls, request):
        if not os.getenv('FLECTRA_BUS_PUBLIC_SAMESITE_WS'):
            return
