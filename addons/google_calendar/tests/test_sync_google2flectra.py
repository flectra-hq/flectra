...
from flectra.addons.google_calendar.utils.google_calendar import GoogleEvent, GoogleCalendarService
from flectra import Command, tools
...
class TestSyncGoogle2Flectra(TestSyncGoogle):
    ...
    def test_attendee_cancelled(self):
        ...
        self.env['calendar.event'].with_user(user)._sync_google2flectra(gevent)
    ...
    def test_recurrence_moved(self):
        ...
        values = {
            ...
            "attendees": [
                {
                    "email": "flectrabot@example.com", "responseStatus": "accepted",
                },
            ],
            'updated': self.now,
            'guestsCanModify': True,
    ...
    def test_recurrence_no_duplicate(self):
        ...
        self.env['calendar.recurrence']._sync_google2flectra(google_events)
    ...
    def test_several_attendee_have_the_same_mail(self):
        """
        In google, One mail = One attendee but on Flectra, some partners could share the same mail
        ...
        self.assertEqual(mails, ['dalton@example.com', 'flectrabot@example.com'])
        gevent = GoogleEvent([{
            'id': google_id,
            'description': 'coucou',
            "updated": self.now,
    ...
    def test_several_users_have_the_same_mail(self):
        ...
        event = self.env['calendar.event']._sync_google2flectra(GoogleEvent([values]))
    ...
    def test_event_with_meeting_url(self):
        values = {
            ...
            'conferenceData': {
                'entryPoints': [{
                    ...
                    'uri': 'https://meet.google.com/flectra-random-test',
                    'label': 'meet.google.com/flectra-random-test'
                }, {
                    'entryPointType': 'more',
                    'uri':'https://tel.meet/flectra-random-test?pin=42424242424242',
                    'pin':'42424242424242'
                }]
            }
        }
        ...
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
        self.assertTrue(event, "It should have created an event")
        self.assertEqual(event.videocall_location, 'https://meet.google.com/flectra-random-test')
        self.assertGoogleAPINotCalled()
    ...
    def test_event_with_local_videocall(self):
        ...
        event = self.env['calendar.event'].create({
            ...
            'videocall_location': 'https://meet.google.com/flectra_local_videocall',
        })
        values = {
            'id': google_id,
            'status': 'confirmed',
        ...
        self.env['calendar.event']._sync_google2flectra(gevent)
        self.assertEqual(event.videocall_location, 'https://meet.google.com/flectra_local_videocall')

        # now google has meet URL and make sure local video call is updated accordingly
        values['conferenceData']['entryPoints'][0]['uri'] = 'https://meet.google.com/flectra-random-test'
        gevent = GoogleEvent([values])
        self.env['calendar.event']._sync_google2flectra(gevent)
        self.assertEqual(event.videocall_location, 'https://meet.google.com/flectra-random-test')
        self.assertGoogleAPINotCalled()
    ...
    def test_partner_order(self):
        self.private_partner.email = "internal_user@flectrahq.com"
        ...
        values = {
            'id': 'oj44nep1ldf8a3ll02uip0c9aa',
            'description': 'Small mini desc',
            'organizer': {'email': 'internal_user@flectrahq.com'},
            'summary': 'Pricing new update',
            'visibility': 'public',
            'attendees': [{
                'displayName': 'Mitchell Admin',
        ...
        self.env['calendar.event'].with_user(user)._sync_google2flectra(GoogleEvent([values]))
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
    ...
    def test_recurrence_range_start_date_in_other_dst_period(self):
        ...
        self.env['calendar.recurrence']._sync_google2flectra(google_event)
    ...
    def test_recurrence_edit_specific_event(self):
        ...
        self.env['calendar.recurrence']._sync_google2flectra(recurrent_events)
        events = self.env["calendar.event"].search(
            [
        ...
        self.env['calendar.event']._sync_google2flectra(specific_event)
        events = self.env["calendar.event"].search(
            [
                ("name", "=", specific_event.summary),
                ("start", ">=", day_start),
    ...
    def test_event_guest_modify_permission(self):
        ...
        This test states that Flectra Calendar:
        ...
        # Sync events from Google to Flectra and get them after sync.
        self.env['calendar.event']._sync_google2flectra(GoogleEvent([not_editable_event_values, editable_event_values]))
        ...
        # Assert that event is created in Flectra with proper values for guests_readonly variable.
    ...
    def test_attendee_status_is_not_updated_when_syncing_and_time_data_is_not_changed(self):
        ...
        self.env['calendar.recurrence'].with_user(other_user)._sync_google2flectra(GoogleEvent(values))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
    ...
    def test_accepting_recurrent_event_with_this_event_option_synced_by_attendee(self, mock_get_events):
        ...
        Ensure that event is accepeted by attendee in Flectra.
        """
        recurrence_id = "abcd1"
        recurrence = self.generate_recurring_event(
            mock_dt="2024-04-20",
    ...
    def test_accepting_recurrent_event_with_all_events_option_synced_by_attendee(self, mock_get_events):
        ...
        Ensure that all events are accepeted by attendee in Flectra.
        """
        recurrence_id = "abcd3"
        recurrence = self.generate_recurring_event(
            mock_dt="2024-04-20",
    ...
    def test_accepting_recurrent_event_with_following_events_option_synced_by_attendee(self, mock_get_events):
        ...
        Ensure that affected events are accepeted by attendee in Flectra.
        """
        recurrence_id = "abcd5"
        recurrence = self.generate_recurring_event(
            mock_dt="2024-04-20",
    ...
    def test_keep_organizer_attendee_writing_recurrence_from_google(self):
        ...
        self.env['calendar.recurrence'].with_user(self.organizer_user)._sync_google2flectra(GoogleEvent(values))
        events = recurrence.calendar_event_ids.sorted('start')
