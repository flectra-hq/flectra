...
class TestSyncFlectra2Google(TestSyncGoogle):
    ...
    def test_event_creation_perf(self):
        ...
        with self.assertQueryCount(__system__=615):
            ...
            events._sync_flectra2google(self.google_service)
    ...
    def test_event_allday_creation(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'flectrabot@example.com', 'self': True},
            'attendees': [{'email': 'flectrabot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_flectra_id' % self.env.cr.dbname: event.id}},
            'transparency': 'opaque',
        })
    ...
    def test_all_event_updated(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'extendedProperties': {'shared': {'%s_flectra_id' % self.env.cr.dbname: new_recurrence.id}},
            'reminders': {'overrides': [], 'useDefault': False},
            'visibility': 'public',
            'transparency': 'opaque',
        }, timeout=3)
    ...
    def test_recurrence_delete_single_events(self):
        ...
        self.assertGoogleEventPatched(event_1.google_id, {
            ...
            'organizer': {'email': 'flectrabot@example.com', 'self': True},
            'attendees': [{'email': 'flectrabot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_flectra_id' % self.env.cr.dbname: event_1.id}},
            'reminders': {'overrides': [], 'useDefault': False},
            'visibility': 'public',
            'status': 'cancelled',
            'transparency': 'opaque',
    ...
    def test_create_event_with_sync_config_paused(self):
        ...
        record._sync_flectra2google(self.google_service)
    ...
    def test_update_synced_event_with_sync_config_paused(self):
        ...
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertEqual(record.name, "Updated Event", "Assert that event name was updated in Flectra Calendar")
        self.assertTrue(record.need_sync, "Sync variable must be true for updating event when sync re-activates")
    ...
    def test_delete_synced_event_with_sync_config_paused(self):
        """
        Deletes a synced event with synchronization paused, event must be archived in Flectra and
        ...
        # Assert that synchronization is paused, delete wasn't called and record was archived in Flectra.
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertFalse(record.active, "Event must be archived in Flectra after unlinking it")
    ...
    def test_event_sync_after_pause_period(self, mock_sync_request):
        ...
        self.assertGoogleEventInserted({
            ...
            'extendedProperties': {'shared': {'%s_flectra_id' % self.env.cr.dbname: record.id}},
        })
    ...
    def test_event_duplication_allday_google_calendar(self):
        ...
        event2 = event.copy()
        event2._sync_flectra2google(self.google_service)
        self.assertGoogleEventInsertedMultiTime({
            **event_response_data,
            'extendedProperties': {'shared': {'%s_flectra_id' % self.env.cr.dbname: event2.id}},
        })
