...
from flectra import api, models, Command
from flectra.tools import email_normalize
...
class RecurrenceRule(models.Model):
    ...
    def _write_from_google(self, gevent, vals):
        ...
        # event_tz is written on event in Google but on recurrence in Flectra
        ...
        for flectra_attendee_email in set(existing_attendees.mapped('email')):
            # Sometimes, several partners have the same email. Remove old attendees except organizer, otherwise the events will disappear.
            if email_normalize(flectra_attendee_email) not in emails:
                attendees = existing_attendees.exists().filtered(lambda att: att.email == email_normalize(flectra_attendee_email) and att.partner_id not in organizers_partner_ids)
    ...
    def _create_from_google(self, gevents, vals_list):
        ...
        for gevent, vals in zip(gevents, vals_list):
            base_values = dict(
                self.env['calendar.event']._flectra_values(gevent),  # FIXME default reminders
                need_sync=False,
            )
            # If we convert a single event into a recurrency on Google, we should reuse this event on Flectra
    ...
    @api.model
    def _flectra_values(self, google_recurrence, default_reminders=()):
        return {
