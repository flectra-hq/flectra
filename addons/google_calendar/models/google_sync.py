...
class GoogleSync(models.AbstractModel):
    ...
    def _sync_google2flectra(self, google_events: GoogleEvent, default_reminders=()):
        ...
        for gevent in pending:
            ...
            if not flectra_record_write_date or updated >= pytz.utc.localize(flectra_record_write_date):
                ...
                flectra_record.with_context(context)._write_from_google(gevent, vals)
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from Flectra to Google, False otherwise. """
        is_active_clause = (self._active_name, '=', True) if self._active_name else expression.TRUE_LEAF
