...
flectra.loader.bus.addEventListener("module-started", (e) => {
    if (e.detail.moduleName === "@im_livechat/embed/common/livechat_service") {
        const { LivechatService, FLECTRA_VERSION_KEY } = e.detail.module;
        // Livechat sessions are deleted if they come from a different flectra version than
        ...
        patch(LivechatService.prototype, {
            async initialize() {
                try {
                    ...
                    localStorage.setItem(FLECTRA_VERSION_KEY, serverVersion);
                } catch {
