...
import "@website/libs/zoomflectra/zoomflectra";
        ...
        if (salePage.dataset.ecomZoomAuto) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomFlectra = $image.data("zoomFlectra");
                    if (zoomFlectra) {
                        zoomFlectra.hide();
                        $image.unbind();
                    }
        ...
        if (salePage.dataset.ecomZoomClick) {
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoomflectra-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.flectraEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.flectraEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
