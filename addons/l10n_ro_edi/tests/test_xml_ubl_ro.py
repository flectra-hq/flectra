...
class TestUBLRO(TestUBLCommon):
    ...
    def test_export_invoice(self):
        ...
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_flectra/ciusro_out_invoice.xml')

    def test_export_credit_note(self):
        refund = self.create_move("out_refund")
        attachment = self.get_attachment(refund)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_flectra/ciusro_out_refund.xml')

    def test_export_invoice_different_currency(self):
        self.currency_data['currency'] = self.env.ref('base.USD')
        invoice = self.create_move("out_invoice")
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_flectra/ciusro_out_invoice_different_currency.xml')
    ...
    def test_export_invoice_without_country_code_prefix_in_vat(self):
        ...
        invoice = self.create_move("out_invoice")
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_flectra/ciusro_out_invoice_no_prefix_vat.xml')
