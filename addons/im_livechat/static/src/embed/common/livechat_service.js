...
export const FLECTRA_VERSION_KEY = `${location.origin.replace(
    /:\/{0,2}/g,
    "_"
)}_im_livechat.flectra_version`;
...
export class LivechatService {
    ...
    async initialize() {
        ...
        if (!this.options.isTestChatbot) {
            ...
            const prevFlectraVersion = browser.localStorage.getItem(FLECTRA_VERSION_KEY);
            const currFlectraVersion = init?.flectra_version;
            const visitorUid = this.visitorUid || false;
            const userId = session.user_id || false;
            if (prevFlectraVersion !== currFlectraVersion || (this.savedState && visitorUid !== userId)) {
                this.leaveSession({ notifyServer: false });
            }
            browser.localStorage.setItem(FLECTRA_VERSION_KEY, currFlectraVersion);
        }
