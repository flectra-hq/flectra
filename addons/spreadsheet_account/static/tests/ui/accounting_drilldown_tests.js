...
import { registries, constants } from "@flectra/o-spreadsheet";
...
QUnit.module("spreadsheet_account > Accounting Drill down", { beforeEach }, () => {
    QUnit.test("Create drill down domain", async (assert) => {
        ...
        env.model = model;
        setCellContent(model, "A1", `=FLECTRA.BALANCE("100", 2020)`);
        setCellContent(model, "A2", `=FLECTRA.BALANCE("100", 0)`);
        setCellContent(model, "A3", `=FLECTRA.BALANCE("100", 2020, , , FALSE)`);
        setCellContent(model, "A4", `=FLECTRA.BALANCE("100", 2020, , , )`);
    ...
    QUnit.test("Create drill down domain when date uses a non-standard locale", async (assert) => {
        ...
        setCellContent(model, "A1", '=FLECTRA.BALANCE("100", DATE(2002, 2, 1))');
        await waitForDataSourcesLoaded(model);
        await doMenuAction(cellMenuRegistry, ["move_lines_see_records"], env);
        assert.verifySteps(["spreadsheet_move_line_action"]);
    });
