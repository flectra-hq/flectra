...
QUnit.module("spreadsheet_account > fiscal year", {}, () => {
    QUnit.test("Basic evaluation", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.FISCALYEAR.START("11/11/2020")`);
        setCellContent(model, "A2", `=FLECTRA.FISCALYEAR.END("11/11/2020")`);
    ...
    QUnit.test("with a given company id", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.FISCALYEAR.START("11/11/2020", 1)`);
        setCellContent(model, "A2", `=FLECTRA.FISCALYEAR.END("11/11/2020", 1)`);
        await waitForDataSourcesLoaded(model);
        assert.verifySteps(["get_fiscal_dates"]);
        assert.equal(getEvaluatedCell(model, "A1").formattedValue, "1/1/2020");
        assert.equal(getEvaluatedCell(model, "A2").formattedValue, "12/31/2020");
    ...
    QUnit.test("with a wrong company id", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.FISCALYEAR.START("11/11/2020", 100)`);
        setCellContent(model, "A2", `=FLECTRA.FISCALYEAR.END("11/11/2020", 100)`);
        await waitForDataSourcesLoaded(model);
        assert.verifySteps(["get_fiscal_dates"]);
        assert.equal(
    ...
    QUnit.test("with wrong input arguments", async (assert) => {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=FLECTRA.FISCALYEAR.START("not a number")`);
        setCellContent(model, "A2", `=FLECTRA.FISCALYEAR.END("11/11/2020", "not a number")`);
        assert.equal(
            getEvaluatedCell(model, "A1").error.message,
            "The function FLECTRA.FISCALYEAR.START expects a number value, but 'not a number' is a string, and cannot be coerced to a number."
        );
        assert.equal(
            getEvaluatedCell(model, "A2").error.message,
            "The function FLECTRA.FISCALYEAR.END expects a number value, but 'not a number' is a string, and cannot be coerced to a number."
        );
    });
