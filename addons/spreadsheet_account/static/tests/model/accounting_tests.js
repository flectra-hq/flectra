...
QUnit.module("spreadsheet_account > Accounting", { beforeEach }, () => {
    ...
    QUnit.test("Basic evaluation", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.CREDIT("100", "2022")`);
        setCellContent(model, "A2", `=FLECTRA.DEBIT("100", "2022")`);
        setCellContent(model, "A3", `=FLECTRA.BALANCE("100", "2022")`);
    ...
    QUnit.test("evaluation with reference to a month period", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.CREDIT("100", B1)`);
        setCellContent(model, "A2", `=FLECTRA.DEBIT("100", B1)`);
        setCellContent(model, "A3", `=FLECTRA.BALANCE("100", B1)`);
        await waitForDataSourcesLoaded(model);
        assert.equal(getCellValue(model, "A1"), 16);
        assert.equal(getCellValue(model, "A2"), 42);
        assert.equal(getCellValue(model, "A3"), 26);
    ...
    QUnit.test("Functions with a wrong company id is correctly in error", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.CREDIT("100", "2022", 0, 123456)`);
        await waitForDataSourcesLoaded(model);
        assert.strictEqual(
    ...
    QUnit.test("formula with invalid date", async (assert) => {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=FLECTRA.CREDIT("100",)`);
        setCellContent(model, "A2", `=FLECTRA.DEBIT("100", 0)`);
        setCellContent(model, "A3", `=FLECTRA.BALANCE("100", -1)`);
        setCellContent(model, "A4", `=FLECTRA.BALANCE("100", "not a valid period")`);
        setCellContent(model, "A5", `=FLECTRA.BALANCE("100", 1900)`); // this should be ok
        setCellContent(model, "A6", `=FLECTRA.BALANCE("100", 1900, -1)`);
        setCellContent(model, "A7", `=FLECTRA.DEBIT("100", 1899)`);
        await waitForDataSourcesLoaded(model);
    ...
    QUnit.test("Evaluation with multiple account codes", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.CREDIT("100,200", "2022")`);
        setCellContent(model, "A2", `=FLECTRA.DEBIT("100,200", "2022")`);
        setCellContent(model, "A3", `=FLECTRA.BALANCE("100,200", "2022")`);

        // with spaces
        setCellContent(model, "B1", `=FLECTRA.CREDIT("100 , 200", "2022")`);
        setCellContent(model, "B2", `=FLECTRA.DEBIT("100 , 200", "2022")`);
        setCellContent(model, "B3", `=FLECTRA.BALANCE("100 , 200", "2022")`);
    ...
    QUnit.test("Server requests", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.BALANCE("100", "2022")`);
        setCellContent(model, "A2", `=FLECTRA.CREDIT("100", "01/2022")`);
        setCellContent(model, "A3", `=FLECTRA.DEBIT("100","Q2/2022")`);
        setCellContent(model, "A4", `=FLECTRA.BALANCE("10", "2021")`);
        setCellContent(model, "A5", `=FLECTRA.CREDIT("10", "2022", -1)`); // same payload as A4: should only be called once
        setCellContent(model, "A6", `=FLECTRA.DEBIT("5", "2021", 0, 2)`);
        setCellContent(model, "A7", `=FLECTRA.DEBIT("5", "05/04/2021", 1)`);
        setCellContent(model, "A8", `=FLECTRA.BALANCE("5", "2022",,,FALSE)`);
        setCellContent(model, "A9", `=FLECTRA.BALANCE("100", "05/05/2022",,,TRUE)`);
        await waitForDataSourcesLoaded(model);
    ...
    QUnit.test("Server requests with multiple account codes", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.BALANCE("100,200", "2022")`);
        setCellContent(model, "A2", `=FLECTRA.CREDIT("100,200", "2022")`);
        setCellContent(model, "A3", `=FLECTRA.DEBIT("100,200","2022")`);
        await waitForDataSourcesLoaded(model);
    ...
    QUnit.test("account group formula as input to balance formula", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.ACCOUNT.GROUP("income")`);
        setCellContent(model, "A2", `=FLECTRA.BALANCE(A1, 2022)`);
    ...
    QUnit.test("two concurrent requests on different accounts", async (assert) => {
        ...
        setCellContent(model, "A1", `=FLECTRA.ACCOUNT.GROUP("income")`);
        setCellContent(model, "A2", `=FLECTRA.BALANCE(A1, 2022)`); // batched only when A1 resolves
        setCellContent(model, "A3", `=FLECTRA.BALANCE("100", 2022)`); // batched directly
        assert.equal(getCellValue(model, "A1"), "Loading...");
        assert.equal(getCellValue(model, "A2"), "Loading...");
    ...
    QUnit.test("date with non-standard locale", async (assert) => {
        ...
        setCellContent(model, "A2", "=FLECTRA.BALANCE(100, A1)");
        setCellContent(model, "A3", "=FLECTRA.CREDIT(100, A1)");
        setCellContent(model, "A4", "=FLECTRA.DEBIT(100, A1)");
        await waitForDataSourcesLoaded(model);
