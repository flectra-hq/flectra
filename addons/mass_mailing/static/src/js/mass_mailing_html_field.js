...
import { getRangePosition } from '@web_editor/js/editor/flectra-editor/src/utils/utils';
import { utils as uiUtils } from "@web/core/ui/ui_service";
import { useSubEnv, status, markup } from "@flectra/owl";
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges(...args) {
        ...
        this._pendingCommitChanges = (async () => {
            ...
            this.wysiwyg.flectraEditor.historyPauseSteps();
            ...
            this.wysiwyg.flectraEditor.observerUnactive('toInline');
            ...
            iframe.remove();
            this.wysiwyg.flectraEditor.observerActive('toInline');
            const inlineHtml = editableClone.innerHTML;
            $editorEnable.addClass('editor_enable');
            this.wysiwyg.flectraEditor.historyUnpauseSteps();
            this.wysiwyg.flectraEditor.historyRevertCurrentStep();
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.flectra-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.flectraEditor.observerFlush();
        this.wysiwyg.flectraEditor.historyReset();
    ...
    async _onSnippetsLoaded() {
        ...
        $snippetsSideBar.on('click', '.o_codeview_btn', () => {
            this.wysiwyg.flectraEditor.observerUnactive();
            $codeview.toggleClass('d-none');
            this.wysiwyg.getEditable().toggleClass('d-none');
            this.wysiwyg.flectraEditor.observerActive();

            if ($codeview.hasClass('d-none')) {
                this.wysiwyg.setValue(this._getCodeViewValue($codeview[0]));
                this.wysiwyg.flectraEditor.sanitize();
                this.wysiwyg.flectraEditor.historyStep(true);
            } else {
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the Flectra editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();
                // Update undo/redo buttons
                this.wysiwyg.flectraEditor.dispatchEvent(new Event('historyStep'));

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.flectraEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.flectraEditor.editable.querySelector('p');
                if (p && selection) {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.flectraEditor.resetContent($newLayout[0].outerHTML);
    ...
    async _lazyloadWysiwyg() {
        ...
        const wysiwygModule = await flectra.loader.modules.get('@mass_mailing/js/mass_mailing_wysiwyg');
