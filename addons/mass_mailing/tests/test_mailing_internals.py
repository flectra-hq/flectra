...
from flectra.addons.base.tests.test_ir_cron import CronMixinCase
from flectra.addons.mass_mailing.tests.common import MassMailCommon
from flectra.exceptions import ValidationError
from flectra.sql_db import Cursor
from flectra.tests.common import users, Form, HttpCase, tagged
...
class TestMassMailValues(MassMailCommon):
    ...
    def test_mailing_body_cropped_vml_image(self):
        ...
        def patched_images_to_urls(self, b64images):
            ...
            if len(urls) == 1:
                ...
                return []
        with patch("flectra.addons.mass_mailing.models.mailing.MassMailing._get_image_by_url",
                   new=patched_get_image), \
             patch("flectra.addons.mass_mailing.models.mailing.MassMailing._create_attachments_from_inline_images",
    ...
    def test_mailing_body_inline_image(self):
        ...
        def patched_images_to_urls(self, b64images):
            ...
            return urls
        with patch("flectra.addons.mass_mailing.models.mailing.MassMailing._create_attachments_from_inline_images",
                   new=patched_images_to_urls):
            mailing = self.env['mailing.mailing'].create({
                    'name': 'Test',
                    'subject': 'Test',
    ...
    def test_mailing_computed_fields_domain_w_filter(self):
        ...
        filter_1, filter_2, filter_3 = self.env['mailing.filter'].create([
            ...
            {'name': 'Email based',
             'mailing_domain' : [('email', 'ilike', 'info@flectrahq.com')],
             'mailing_model_id': self.env['ir.model']._get('res.partner').id,
            }
        ])
        ...
        mailing.mailing_domain = "[('email', 'ilike', 'info_be@flectrahq.com')]"
...
class TestMassMailFeatures(MassMailCommon, CronMixinCase):
    ...
    def test_mailing_shortener(self):
        mailing = self.env['mailing.mailing'].create({
            ...
            'body_html': """<div>
Hi,
<t t-set="url" t-value="'www.flectrahq.com'"/>
<t t-set="httpurl" t-value="'https://www.flectra.eu'"/>
Website0: <a id="url0" t-attf-href="https://www.flectra.tz/my/{{object.name}}">https://www.flectra.tz/my/<t t-esc="object.name"/></a>
