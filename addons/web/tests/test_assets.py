...
class TestAssetsGenerateTimeCommon(flectra.tests.TransactionCase):
...
@flectra.tests.tagged('post_install', '-at_install', '-standard', 'test_assets')
class TestPregenerateTime(HttpCase):
    ...
    def test_logs_pregenerate_time(self):
        ...
        with self.profile(collectors=['sql', flectra.tools.profiler.PeriodicCollector(interval=0.01)], disable_gc=True):
            self.env['ir.qweb']._pregenerate_assets_bundles()
        duration = time.time() - start
        _logger.info('All bundle checked in %.2fs', duration)

@flectra.tests.tagged('post_install', '-at_install', '-standard', 'assets_bundle')
...
class TestLoad(HttpCase):
    def test_assets_already_exists(self):
        ...
        _save_attachment = flectra.addons.base.models.assetsbundle.AssetsBundle.save_attachment
        ...
        with patch('flectra.addons.base.models.assetsbundle.AssetsBundle.save_attachment', save_attachment):
