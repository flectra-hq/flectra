...
QUnit.module("Views", (hooks) => {
    ...
    QUnit.test("buttons in form view", async function (assert) {
        ...
        assert.verifyErrors(["Flectra Server Error"]);
    });
    ...
    QUnit.test("onchange returns an error", async function (assert) {
        ...
        registry.category("error_dialogs").add("flectra.exceptions.UserError", WarningDialog);
    ...
    QUnit.test("proper stringification in debug mode tooltip", async function (assert) {
        patchWithCleanup(flectra, { debug: true });
                ...
                if (obj.display_name === "first line") {
                    if (onChangeCount === 0) {
                        ...
                        throw makeErrorFromResponse({
                            ...
                            data: {
                                name: `flectra.exceptions.${"UserError"}`,
                                debug: "traceback",
                                arguments: [],
                                context: {},
                                message: "error",
