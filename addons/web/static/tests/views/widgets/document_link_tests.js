...
QUnit.module("Widgets", (hooks) => {
    ...
    QUnit.test("documentation_link: relative path", async function (assert) {
        ...
        assert.hasAttrValue(
            ...
            "href",
            "https://www.flectrahq.com/documentation/1.0/applications/technical/web/settings/this_is_a_test.html"
        );
    });
    QUnit.test("documentation_link: absoluth path (http)", async function (assert) {
        await makeView({
            ...
            arch: `
                <form>
                    <field name="bar"/>
                    <widget name="documentation_link"  path="http://www.flectrahq.com/"/>
                </form>`,
        });

        assert.hasAttrValue(target.querySelector(".o_doc_link"), "href", "http://www.flectrahq.com/");
    });
    QUnit.test("documentation_link: absoluth path (https)", async function (assert) {
        await makeView({
            ...
            arch: `
                <form>
                    <field name="bar"/>
                    <widget name="documentation_link"  path="https://www.flectrahq.com/"/>
                </form>`,
        });

        assert.hasAttrValue(target.querySelector(".o_doc_link"), "href", "https://www.flectrahq.com/");
    });
});
