...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Flectra" });
    assert.strictEqual(env.services.title.current, "Flectra");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Flectra", chat: null });
    assert.strictEqual(env.services.title.current, "Flectra");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Flectra - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Flectra", action: "Import" });
    assert.strictEqual(env.services.title.current, "Flectra - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Flectra", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Flectra - Import"); // parts is a copy!
});
