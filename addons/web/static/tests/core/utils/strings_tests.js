...
QUnit.module("utils", () => {
    ...
    QUnit.test("escapeRegExp", (assert) => {
        ...
        assert.deepEqual(escapeRegExp("[wowl]"), "\\[wowl\\]");
        assert.deepEqual(escapeRegExp("[wowl.flectra]"), "\\[wowl\\.flectra\\]");
        assert.deepEqual(
            escapeRegExp("^flectra.define([.]*)$"),
            "\\^flectra\\.define\\(\\[\\.\\]\\*\\)\\$"
        );
        assert.deepEqual(
            escapeRegExp("[.*+?^${}()|[]\\"),
    ...
    QUnit.test("escape", (assert) => {
        ...
        assert.strictEqual(
            escape(`<a href="https://www.flectrahq.com">flectra<a>`),
            `&lt;a href=&quot;https://www.flectrahq.com&quot;&gt;flectra&lt;a&gt;`
        );
        assert.strictEqual(
            escape(`<a href='https://www.flectrahq.com'>flectra<a>`),
            `&lt;a href=&#x27;https://www.flectrahq.com&#x27;&gt;flectra&lt;a&gt;`
        ...
        assert.strictEqual(
            escape("<a href='https://www.flectrahq.com'>Flectra`s website<a>"),
            `&lt;a href=&#x27;https://www.flectrahq.com&#x27;&gt;Flectra&#x60;s website&lt;a&gt;`
        );
    });
    ...
    QUnit.test("isEmail", (assert) => {
        ...
        assert.notOk(isEmail("test@flectra"));
        assert.notOk(isEmail("test@flectra@flectrahq.com"));
        assert.notOk(isEmail("te st@flectrahq.com"));

        assert.ok(isEmail("test@flectrahq.com"));
    });
});
