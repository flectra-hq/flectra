...
import { onMounted } from "@flectra/owl";
let serverData;
let target;
...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("flags field of ir.actions.act_window is used", async function (assert) {
        // more info about flags field : https://github.com/flectra/flectra/commit/c9b133813b250e89f1f61816b0eabfb9bee2009d
        assert.expect(6);
    ...
    QUnit.test("debugManager is active for (legacy) views", async function (assert) {
        ...
        patchWithCleanup(flectra, { debug: "1" });
    ...
    QUnit.test("window action in target new fails (onchange)", async (assert) => {
        ...
        registry
            ...
            .add("flectra.exceptions.ValidationError", WarningDialogWait);
    ...
    QUnit.test("pushState also changes the title of the tab", async (assert) => {
        ...
        assert.strictEqual(titleService.current, '{"zopenerp":"Flectra","action":"Partners"}');
        await click(target.querySelector(".o_data_row .o_data_cell"));
        assert.strictEqual(titleService.current, '{"zopenerp":"Flectra","action":"First record"}');
        await click(target.querySelector(".o_pager_next"));
        assert.strictEqual(titleService.current, '{"zopenerp":"Flectra","action":"Second record"}');
    });
