...
(function () {
        ...
        define(name, deps, factory) {
            ...
            if (!this.factories.has(name)) {
                this.factories.set(name, {
                    deps,
                    fn: factory,
                    ignoreMissingDeps: globalThis.__flectraIgnoreMissingDependencies,
                });
                this.addJob(name);
    ...
    if (!globalThis.flectra) {
        globalThis.flectra = {};
    }
    const flectra = globalThis.flectra;
    if (flectra.debug && !new URLSearchParams(location.search).has("debug")) {
        // remove debug mode if not explicitely set in url
        flectra.debug = "";
    }

    const loader = new ModuleLoader();
    flectra.define = loader.define.bind(loader);

    flectra.loader = loader;
})();
