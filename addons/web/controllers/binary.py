...
try:
    ...
    from flectra.tools._vendor.send_file import send_file

import flectra
import flectra.modules.registry
from flectra import SUPERUSER_ID, _, http
...
Flectra is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Flectra is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if flectra.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=flectra.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or flectra.SUPERUSER_ID
