...
from flectra.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from flectra.service import security
from flectra.sql_db import Cursor
from flectra.tools import float_compare, single_email_re
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    ...
    def _callSetUp(self):
        ...
        # See https://github.com/flectra/flectra/pull/107572 for more info.
...
class _ErrorCatcher(list):
    ...
    def _complete_traceback(self, initial_tb):
        ...
        # remove traceback root part (flectra_bin, main, loading, ...), as
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.env = api.Environment(self.cr, flectra.SUPERUSER_ID, {})
...
class HttpCaseCommon(BaseCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        self.xmlrpc_url = url_8 = 'http://%s:%d/xmlrpc/2/' % (HOST, flectra.tools.config['http_port'])
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True):
        self.env['base'].flush()
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, flectra.tools.config['http_port'], url)
        if data or files:
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, **kw):
        ...
        try:
            ...
            base_url = "http://%s:%s" % (HOST, flectra.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "flectra.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "flectra.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~flectra.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~flectra.fields.Many2many` fields, the field itself is a
    :class:`~flectra.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~flectra.fields.One2many` are reified as
    :class:`~flectra.tests.common.O2MProxy`.

    Because the :class:`~flectra.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~flectra.tests.common.O2MProxy.new` and
    :meth:`~flectra.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: flectra.models.Model
    ...
    :type view: int | str | flectra.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~flectra.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~flectra.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = flectra.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = flectra.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return flectra.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return flectra.fields.Date.to_string(value)
    return value
