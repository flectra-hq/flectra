...
from flectra.http import BadRequest
from flectra.modules import module
from flectra.modules.registry import Registry
from flectra.service import security
from flectra.sql_db import BaseCursor, Cursor
from flectra.tools import float_compare, single_email_re, profiler, lower_logging, SQL
from flectra.tools.misc import find_in_path, mute_logger
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 17.0: flectra.tests.common.Form is deprecated, use flectra.tests.Form",
...
def standalone(*tags):
    ...
    """
    def register(func):
        # register func by flectra module name
        if func.__module__.startswith('flectra.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most flectra operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('flectra.addons.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Flectra-specific code. This class is abstract and
    ...
    def run(self, result):
        ...
        if getattr(testMethod, '_retry', True) and getattr(self, '_retry', True):
            tests_run_count = int(os.environ.get('FLECTRA_TEST_FAILURE_RETRIES', 0)) + 1
        else:
            tests_run_count = 1
    ...
    def debug_mode(self):
        ...
        with patch('flectra.models.BaseModel.user_has_groups', user_has_groups):
            yield
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('flectra.sql_db.Cursor.execute', execute):
            with patch('flectra.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/flectra/addons/" in filename:
                        filename = filename.rsplit("/flectra/addons/", 1)[1]
                    if count > expected:
    ...
    def check_test_cursor(self, operation):
        if flectra.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {flectra.modules.module.current_test.canonical_tag}"
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(flectra.modules.registry._logger.name)
    ...
    def _gc_filestore(cls):
        ...
        with flectra.registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, flectra.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.env = api.Environment(cls.cr, flectra.SUPERUSER_ID, {})
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(flectra.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_class, headless=True):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_flectra')

        otc = flectra.tools.config
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{flectra.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('flectra.service.http.request.')]
    ...
    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            flectra.http.root.session_store.delete(self.session)

        self.session = session = flectra.http.root.session_store.new()
        session.update(flectra.http.get_default_session(), db=get_db_name())
        session.context['lang'] = flectra.http.DEFAULT_LANG
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, cpu_throttling=None, **kw):
        ...
        try:
            ...
            cpu_throttling_os = os.environ.get('FLECTRA_BROWSER_CPU_THROTTLING') # used by dedicated runbot builds
    ...
    @classmethod
    def base_url(cls):
        return f"http://{HOST}:{flectra.tools.config['http_port']}"
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "flectra.startTour('%s', %s)" % (tour_name, json.dumps(options)))
        ready = kwargs.pop('ready', "flectra.isTourReady('%s')" % tour_name)
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('flectra.http.Request._get_profiler_context_manager', route_profiler))
...
def tagged(*tags):
    ...
    By default, all Test classes from flectra.tests.common have a test_tags
