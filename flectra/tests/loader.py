...
def get_test_modules(module):
    ...
    results = _get_tests_modules('flectra.addons', module)

    try:
        importlib.import_module('flectra.upgrade.%s' % module)
    except ImportError:
        pass
    else:
...
def _get_upgrade_test_modules(module):
    upg = importlib.import_module("flectra.upgrade")
    for path in map(Path, upg.__path__):
        for test in (path / module / "tests").glob("test_*.py"):
            spec = importlib.util.spec_from_file_location(f"flectra.upgrade.{module}.tests.{test.stem}", test)
            if not spec:
                continue
