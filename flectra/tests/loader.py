...
from .suite import FlectraSuite
from .result import FlectraTestResult


def get_test_modules(module):
    """ Return a list of module for the addons potentially containing tests to
    feed unittest.TestLoader.loadTestsFromModule() """
    results = _get_tests_modules(importlib.util.find_spec(f'flectra.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"flectra.upgrade.{module}",
        f"flectra.addons.{module}.migrations",
        f"flectra.addons.{module}.upgrades",
    )
...
def run_suite(suite, module_name=None, global_report=None):
    ...
    results = FlectraTestResult(global_report=global_report)
    suite(results)
