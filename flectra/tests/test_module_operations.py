...
import flectra
from flectra.tools import config, topological_sort, unique
from flectra.netsvc import init_logger
from flectra.tests import standalone_tests
import flectra.tests.loader

_logger = logging.getLogger('flectra.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Flectra modules. Prefer the 'cycle' subcommand to"
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        flectra.tools.config['addons_path'] = ','.join([args.addons_path, flectra.tools.config['addons_path']])
        if args.data_dir:
            flectra.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'flectra.modules.loading': {'level': 'CRITICAL'},
            'flectra.sql_db': {'level': 'CRITICAL'},
            'flectra.models.unlink': {'level': 'WARNING'},
            'flectra.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
