...
class Cursor(BaseCursor):
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # Flectra Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri):
    ...
    :rtype: (str, dict)
    """
    if 'FLECTRA_PGAPPNAME' in os.environ:
        # Using manual string interpolation for security reason and trimming at default NAMEDATALEN=63
        app_name = os.environ['FLECTRA_PGAPPNAME'].replace('{pid}', str(os.getpid()))[0:63]
    else:
        app_name = "flectra-%d" % os.getpid()
...
def db_connect(to, allow_uri=False):
    ...
    if _Pool is None:
        _Pool = ConnectionPool(int(
            flectra.evented and tools.config['db_maxconn_gevent']
