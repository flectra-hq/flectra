...
class Obfuscate(Command):
    """Obfuscate data in a given flectra database"""
    def __init__(self):
        super().__init__()
        self.cr = None
    ...
    def set_pwd(self, pwd):
        ...
        self.cr.execute("INSERT INTO ir_config_parameter (key, value) VALUES ('flectra_cyph_pwd', 'flectra_cyph_'||encode(pgp_sym_encrypt(%s, %s), 'base64')) ON CONFLICT(key) DO NOTHING", [pwd, pwd])
    ...
    def check_pwd(self, pwd):
        ...
        try:
            qry = sql.SQL("SELECT {uncypher_pwd} FROM ir_config_parameter WHERE key='flectra_cyph_pwd'").format(uncypher_pwd=uncypher_pwd)
    ...
    def clear_pwd(self):
        ...
        self.cr.execute("DELETE FROM ir_config_parameter WHERE key='flectra_cyph_pwd' ")

    def cypher_string(self, sql_field):
        # don't double cypher fields
        return sql.SQL("""CASE WHEN starts_with({field_name}, 'flectra_cyph_') THEN {field_name} ELSE 'flectra_cyph_'||encode(pgp_sym_encrypt({field_name}, %(pwd)s), 'base64') END""").format(field_name=sql_field)

    def uncypher_string(self, sql_field):
        return sql.SQL("""CASE WHEN starts_with({field_name}, 'flectra_cyph_') THEN pgp_sym_decrypt(decode(substring({field_name}, 11)::text, 'base64'), %(pwd)s) ELSE {field_name} END""").format(field_name=sql_field)
    ...
    def run(self, cmdargs):
        parser = flectra.tools.config.parser
        ...
        try:
            opt = flectra.tools.config.parse_config(cmdargs)
            if not opt.pwd:
            ...
            self.dbname = flectra.tools.config['db_name']
            self.registry = flectra.registry(self.dbname)
