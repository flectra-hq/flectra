...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        while True:
            conn = flectra.sql_db.db_connect('postgres')
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = flectra.tests.result._logger
...
class WorkerCron(Worker):
    ...
    def start(self):
        ...
        dbconn = flectra.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = list(flectra.conf.server_wide_modules)
...
def load_test_file_py(registry, test_file):
    ...
    from flectra.tests.suite import FlectraSuite
...
def start(preload=None, stop=False):
    ...
    if flectra.evented:
        server = GeventServer(flectra.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(flectra.http.root)
        ...
        server = ThreadedServer(flectra.http.root)
