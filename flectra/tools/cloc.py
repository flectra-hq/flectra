# -*- coding: utf-8 -*-
# Part of Odoo, Flectra. See LICENSE file for full copyright and licensing details.import ast
import pathlib
import os
import re
import shutil
...
import flectra
from flectra.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in flectra.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [flectra.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(flectra.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be flectra/studio unless it comes from an imported module install
