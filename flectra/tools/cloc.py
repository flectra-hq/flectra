...
class Cloc(object):
    ...
    def count_customization(self, env):
        ...
        for a in env['ir.actions.server'].browse(data.keys()):
            self.book(data[a.id] or "flectra/studio", "ir.actions.server/%s: %s" % (a.id, a.name), self.parse_py(a.code))
        ...
        for f in env['ir.model.fields'].browse(data.keys()):
            self.book(data[f.id] or "flectra/studio", "ir.model.fields/%s: %s" % (f.id, f.name), self.parse_py(f.compute))
