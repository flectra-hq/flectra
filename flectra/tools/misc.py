...
def file_path(file_path, filter_ext=('',), env=None):
    ...
    addons_paths = flectra.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None, env=None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/flectra/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def ignore(*exc):
    warnings.warn("Since 16.0 `flectra.tools.ignore` is replaced by `contextlib.suppress`.", DeprecationWarning, stacklevel=2)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = flectra.fields.Datetime.from_string(value)
    else:
        timestamp = value
