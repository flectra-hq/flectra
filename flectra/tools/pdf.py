...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    """ Add a banner on a PDF in the upper right corner, with Flectra's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Flectra's logo in the banner.
    ...
    flectra_logo = Image.open(file_open('base/static/img/main_partner-image.png', mode='rb'))
    flectra_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        can.setFillColor(flectra_color)
        ...
        logo and can.drawImage(
            ImageReader(flectra_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
