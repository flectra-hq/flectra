...
PYTHON_TRANSLATION_COMMENT = 'flectra-python'

# translation used for javascript code in web client
JAVASCRIPT_TRANSLATION_COMMENT = 'flectra-javascript'
# used to notify web client that these translations should be loaded in the UI
# deprecated comment since Flectra 16.0
        ...
        try:
            ...
            if lang and lang != 'en_US':
                if not module:
                    ...
                    path_info = flectra.modules.get_resource_from_path(path)
...
def _extract_translatable_qweb_terms(element, callback):
    ...
    for el in element:
        ...
        if (el.tag.lower() not in SKIPPED_ELEMENTS
            ...
            # https://github.com/flectra/owl/blob/master/doc/reference/component.md#composition
...
class TranslationReader:
    def __init__(self, cr, lang=None):
        ...
        self.env = flectra.api.Environment(cr, flectra.SUPERUSER_ID, {})
        self._to_translate = []

    def __iter__(self):
        for module, source, name, res_id, ttype, comments, _record_id, value in self._to_translate:
            yield (module, ttype, name, res_id, source, encode(flectra.tools.ustr(value)), comments)
...
class TranslationModuleReader(TranslationReader):
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._path_list = [(path, True) for path in flectra.addons.__path__]
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'flectra.tools.translate:babel_extract_qweb',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if fnmatch.fnmatch(root, '*/data/*'):
                    for fname in fnmatch.filter(files, '*_dashboard.json'):
                        self._babel_extract_terms(fname, path, root, 'flectra.tools.translate:extract_spreadsheet_terms',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if not recursive:
...
def get_po_paths_env(module_name: str, lang: str, env: flectra.api.Environment | None = None):
...
def _get_translation_upgrade_queries(cr, field):
    ...
    """
    Model = flectra.registry(cr.dbname)[field.model_name]
