...
class UpgradeHook(object):
    ...
    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^flectra\.addons\.base\.maintenance\.migrations\b", fullname):
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(flectra.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'flectra.addons.{module_name}'
