...
class TestHttpSession(TestHttpBase):
    ...
    def test_session1_default_session(self):
        ...
        with patch.object(flectra.http.root.session_store, 'save') as mock_save:
    ...
    def test_session3_logout_15_0_geoip(self):
        ...
        with self.assertLogs('flectra.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:flectra.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    def test_session7_serializable(self):
        ...
        # Values forbidden by flectra, raising a warning
        for value in [
            str,
            int,
            float,
            ...
            with self.assertLogs(level="WARNING"):
                ...
                self.assertFalse(flectra.http.root.session_store.session_class({'foo': value}, 1234).foo)
    ...
    def test_session8_logout(self):
        ...
        self.assertTrue(flectra.http.root.session_store.get(sid), "session should exist")
        self.url_open('/web/session/logout', allow_redirects=False).raise_for_status()
        self.assertFalse(flectra.http.root.session_store.get(sid), "session should not exist")
    ...
    def test_session9_explicit_session(self):
        ...
        with self.assertLogs('flectra.http') as capture:
        ...
        self.assertRegex(capture.output[0],
            r"^WARNING:flectra.http:<function flectra\.addons\.\w+\.controllers\.\w+\.logout> "
