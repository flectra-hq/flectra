...
import flectra
from flectra.http import root, content_disposition
from flectra.tests import tagged
from flectra.tests.common import HOST, new_test_user, get_db_name, BaseCase
from flectra.tools import config, file_path, parse_version
from flectra.addons.test_http.controllers import CT_JSON

from flectra.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc1_reverse_proxy(self):
        # client <-> reverse-proxy <-> flectra
        client_ip = '127.0.0.16'
        reverseproxy_ip = gethostbyname(HOST)
        host = 'mycompany.flectrahq.com'
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = flectra.tools.misc.DotDict(db=False)
            return flectra.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'flectrahq.com/{uri}', host='flectrahq.com'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://flectrahq.com/{uri}', host='flectrahq.com'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'flectrahq.com/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://flectrahq.com/{uri}'), "No host allowed")
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'flectrahq.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict(flectra.tools.config.options, {'proxy_mode': True}):
    ...
    def test_misc6_upload_file_retry(self):
        from flectra.addons.test_http import controllers  # pylint: disable=C0415
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(flectra.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = flectra.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(res.status_code, 302)
        self.assertEqual(urlparse(res.headers.get('Location', '')).path, '/test_http/ensure_db')
        self.assertEqual(flectra.http.root.session_store.get(res.cookies['session_id']).db, 'basededonnée1')
