...
from flectra import release, SUPERUSER_ID, _
from flectra.http import request
from flectra.tools import (func, misc, transpile_javascript,
    is_flectra_module, SourceMapGenerator, profiler,
    apply_inheritance_specs)
from flectra.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS
from flectra.tools.misc import file_open, file_path
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(f"""
                    ...
                    *******************************************/
                    ...
                    flectra.define('{self.name}.bundle.xml', ['@web/core/registry'], function(require){{
                        'use strict';
        ...
        * `t-extend` deprecated attribute, used by the JavaScript Qweb.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                template = content if content.startswith('<flectra>') else f'<templates>{asset.content}</templates>'
...
class JavascriptAsset(WebAsset):
    ...
    def is_transpiled(self):
        if self._is_transpiled is None:
            self._is_transpiled = bool(is_flectra_module(super().content))
