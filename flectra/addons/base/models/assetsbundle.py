...
from flectra import fields, tools, SUPERUSER_ID
        ...
        return """
            (function (message) {
                ...
                async function onDOMContentLoaded() {
                    var flectra = window.top.flectra;
                    if (!flectra || !flectra.define) {
                        useAlert();
                        return;
                    }
                    ...
                    await new Promise(resolve => {
                        const noLazyTimeout = setTimeout(() => resolve(), 10); // 10 since need to wait for promise resolutions of flectra.define
                        flectra.define('AssetsBundle.PotentialLazyLoading', function (require) {
                            'use strict';
                    ...
                    var alertTimeout = setTimeout(useAlert, 10); // 10 since need to wait for promise resolutions of flectra.define
