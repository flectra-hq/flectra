...
BASE_VERSION = flectra.modules.get_manifest('base')['version']
MAX_FAIL_TIME = timedelta(hours=5)  # chosen with a fair roll of the dice

# custom function to call instead of default PostgreSQL's `pg_notify`
FLECTRA_NOTIFY_FUNCTION = os.getenv('FLECTRA_NOTIFY_FUNCTION', 'pg_notify')
...
class ir_cron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('FLECTRA_NOTIFY_CRON_CHANGES'):
    ...
    def _check_modules_state(cls, cr, jobs):
        ...
        # reset_module_states.
        flectra.modules.reset_modules_state(cr.dbname)
    ...
    def write(self, vals):
        self._try_lock()
        if ('nextcall' in vals or vals.get('active')) and os.getenv('FLECTRA_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list):
        ...
        if min(at_list) <= now or os.getenv('FLECTRA_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
        return triggers

    def _notifydb(self):
        """ Wake up the cron workers
        The FLECTRA_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        ir_cron modification and on trigger creation (regardless of call_at)
        """
        with flectra.sql_db.db_connect('postgres').cursor() as cr:
            query = sql.SQL("SELECT {}('cron_trigger', %s)").format(sql.Identifier(FLECTRA_NOTIFY_FUNCTION))
