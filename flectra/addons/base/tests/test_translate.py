...
from flectra.tools.translate import quote, unquote, xml_translate, html_translate, TranslationImporter, TranslationModuleReader
from flectra.tests.common import TransactionCase, BaseCase, new_test_user, tagged

_stats_logger = logging.getLogger('flectra.tests.stats')
...
class TestLanguageInstall(TransactionCase):
    def test_language_install(self):
        ...
        with patch('flectra.addons.base.models.ir_module.Module._load_module_terms', _load_module_terms):
            wizard.lang_install()
...
class TestTranslationExport(TransactionCase):
    ...
    def test_export_translatable_resources(self):
        ...
        with self.assertNoLogs('flectra.tools.translate', "ERROR"):
...
class TestTranslation(TransactionCase):
    ...
    #     country_1 = Country.create({'name': 'Flectra'})
    #     country_1.with_context(lang='fr_FR').name = 'Flectra_Fr'
    #     country_1.flush_recordset()
    #
    #     country_2 = Country.create({'name': 'Flectra2'})
    #     with self.assertRaises(IntegrityError), mute_logger('flectra.sql_db'):
    #         country_2.name = 'Flectra'
    #         country_2.flush_recordset()
    #
    #     with self.assertRaises(IntegrityError), mute_logger('flectra.sql_db'):
    #         country_3 = Country.create({'name': 'Flectra'})
