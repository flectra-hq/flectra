...
class EmailConfigCase(SavepointCase):
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'flectra.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@flectra.example.com')
        set_param('mail.catchall.domain', 'flectra.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin (admin@test.example.com)" <notification@flectra.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@flectra.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@flectra.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@flectra.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@flectra.example.com>')
        set_param('mail.catchall.domain', 'flectra.example.com')
