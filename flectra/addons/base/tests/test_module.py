...
import flectra.addons
from flectra.modules.module import load_manifest
from flectra.modules.module import get_manifest
from flectra.release import major_version
...
class TestModuleManifest(BaseCase):
    @classmethod
    def setUpClass(cls):
        cls._tmp_dir = tempfile.TemporaryDirectory(prefix='flectra-test-addons-')
        cls.addClassCleanup(cls._tmp_dir.cleanup)
        cls.addons_path = cls._tmp_dir.name

        patcher = patch.object(flectra.addons, '__path__', [cls.addons_path])
        cls.startClassPatcher(patcher)

    def setUp(self):
        self.module_root = tempfile.mkdtemp(prefix='flectra-test-module-', dir=self.addons_path)
    ...
    def test_default_manifest(self):
        ...
        with self.assertNoLogs('flectra.modules.module', 'WARNING'):
    ...
    def test_missing_manifest(self):
        with self.assertLogs('flectra.modules.module', 'DEBUG') as capture:
            manifest = load_manifest(self.module_name)
    ...
    def test_missing_license(self):
        ...
        with self.assertLogs('flectra.modules.module', 'WARNING') as capture:
            manifest = load_manifest(self.module_name)
