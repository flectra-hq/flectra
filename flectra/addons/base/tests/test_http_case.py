...
from flectra.tests.common import ChromeBrowser, HttpCase, tagged
from flectra.tools import config, logging
...
class TestHttpCase(HttpCase):
    ...
    def test_console_error_string(self):
        with self.assertLogs(level='ERROR') as log_catcher:
            with self.assertRaises(AssertionError) as error_catcher:
                ...
                with patch('flectra.tests.common.ChromeBrowser.take_screenshot', return_value=None):
    ...
    def test_console_log_object(self):
        logger = logging.getLogger('flectra')
        level = logger.level
...
class TestRequestRemaining(HttpCase):
    ...
    def test_requests_b(self):
        ...
        with self.assertLogs('flectra.tests.common', level="ERROR") as lc:
            self.main_lock.release()
            _logger.info('B started, waiting for A to finish')
            self.thread_a.join()
        self.assertEqual(lc.output, ['ERROR:flectra.tests.common:Request with path /web/concurrent has been ignored during test as it it does not contain the test_cursor cookie or it is expired. (required "/base/tests/test_http_case.py:TestRequestRemaining.test_requests_b", got "/base/tests/test_http_case.py:TestRequestRemaining.test_requests_a")'])
