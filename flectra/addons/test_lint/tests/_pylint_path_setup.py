...
def register(linter):
    ...
    if addons_path:
        ...
        for p in addons_path.split(os.pathsep):
            ...
            flectra.addons.__path__.append(normp)

def modpath_from_file(filename, path):
    if filename == "./addons":
        return ['flectra', 'addons']
...
class AddonsPackageFinder(spec.Finder):
    """Finder which special cases flectra and flectra.addons, such that they're
    ...
    def find_module(
        ...
        if modname in ('flectra', 'flectra.addons'):
