...
import flectra
from flectra.tests.common import get_db_name, tagged
...
methods_to_sanitize = {
    method_name
    for method_name in dir(flectra.models.BaseModel)
...
def get_flectra_module_name(python_module_name):
    if python_module_name.startswith('flectra.addons.'):
        return python_module_name.split('.')[2]
    if python_module_name == 'flectra.models':
        return 'flectra'
...
class TestLintOverrideSignatures(LintCase):
    def test_lint_override_signature(self):
        ...
        registry = flectra.registry(get_db_name())
        for model_name, model_cls in registry.items():
            for method_name, _ in inspect.getmembers(model_cls, inspect.isroutine):
                ...
                parent_module = get_flectra_module_name(parent_class.__module__)
                ...
                for child_class in reverse_mro:
                    ...
                    child_module = get_flectra_module_name(child_class.__module__)
        ...
        #with open('/tmp/flectra-override-signatures.md', 'w') as f:
