    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('flectra.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'flectra.addons'."
...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~flectra.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~flectra.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~flectra.models.AbstractModel`.
    """
    ...
    _module = None              #: the model's module (in the Flectra sense)
    ...
    .. warning::
      ...
      :attr:`~flectra.models.Model._inherits`-ed models, the inherited field will
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], flectra.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], flectra.fields.One2many):
    ...
    def _compute_display_name(self):
        ...
        it can be made field-dependent using :attr:`~flectra.api.depends` and
        context-dependent using :attr:`~flectra.api.depends_context`.
        """
        if self._rec_name:
    ...
    def _read_group_fill_temporal(self, data, groupby, annoted_aggregates,
        ...
        if fill_from:
            fill_from = flectra.fields.Datetime.to_datetime(fill_from) if isinstance(fill_from, datetime.datetime) else flectra.fields.Date.to_date(fill_from)
        ...
        if fill_to:
            fill_to = flectra.fields.Datetime.to_datetime(fill_to) if isinstance(fill_to, datetime.datetime) else flectra.fields.Date.to_date(fill_to)
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~flectra.addons.base.models.res_company`).

        For :class:`~flectra.addons.base.models.res_users` relational fields,
    ...
    def write(self, vals):
        ...
        * The expected value of a :class:`~flectra.fields.One2many` or
          :class:`~flectra.fields.Many2many` relational field is a list of
          :class:`~flectra.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~flectra.fields.Command.create`,
          :meth:`~flectra.fields.Command.update`,
          :meth:`~flectra.fields.Command.delete`,
          :meth:`~flectra.fields.Command.unlink`,
          :meth:`~flectra.fields.Command.link`,
          :meth:`~flectra.fields.Command.clear`, and
          :meth:`~flectra.fields.Command.set`.
        * For :class:`~flectra.fields.Date` and `~flectra.fields.Datetime`,
    ...
    def ensure_one(self):
        ...
        :raise flectra.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~flectra.addons.base.models.res_company` or int
    ...
    def _populate_factories(self):
        ...
        See :mod:`flectra.tools.populate` for population tools and applications.
