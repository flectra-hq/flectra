...
r"""\
Flectra HTTP layer / WSGI application
...
:class:`~flectra.http.Controller`: class and its companion the
:func:`~flectra.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Flectra http request. The Flectra http request is
...
HTTP methods). See
https://www.flectrahq.com/documentation/17.0/developer/reference/addons/http.html#csrf
for more details.
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~flectra.service.db.list_dbs`:
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': flectra.service.common.dispatch,
        'db': flectra.service.db.dispatch,
        'object': flectra.service.model.dispatch,
    }
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    warnings.warn('flectra.http.send_file is deprecated, please use flectra.http.Stream instead.', DeprecationWarning, stacklevel=2)
    return _send_file(
        filepath_or_fp,
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~flectra.tools.file_path`
        :param filter_ext: See :func:`~flectra.tools.file_path`
    ...
    def get_response(self, as_attachment=None, immutable=None, **send_file_kwargs):
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`flectra.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~flectra.http.Controller` can use the :func:`~flectra.http.route`:
    ...
    :func:`~flectra.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(flectra.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['flectra', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~flectra.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['flectra', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Flectra addons.
        """
        # Controllers defined outside of flectra addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of flectra addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, login=None, password=None):
        ...
        with registry.cursor() as cr:
            env = flectra.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            ...
            request.env = flectra.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~flectra.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> flectra_ip = socket.gethostbyname('flectrahq.com')
        >>> GeoIP(flectra_ip).country.iso_code
        'FR'
    """
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~flectra.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~flectra.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~flectra.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        with contextlib.closing(self.registry.cursor()) as cr:
            self.env = flectra.api.Environment(cr, self.session.uid, self.session.context)
            threading.current_thread().uid = self.env.uid
            try:
                return service_model.retrying(self._serve_ir_http, self.env)
            except Exception as exc:
                if isinstance(exc, HTTPException) and exc.code is None:
                    raise  # bubble up to flectra.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~flectra.http.Response.load` method for the compatible
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in flectra.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + flectra.conf.server_wide_modules, nodb_only=True):
    ...
    def __call__(self, environ, start_response):
        ...
        if flectra.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
